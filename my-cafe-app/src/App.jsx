...
import { getAuth, signInWithEmailAndPassword, signInAnonymously, onAuthStateChanged, signOut, sendSignInLinkToEmail, isSignInWithEmailLink, signInWithEmailLink } from 'firebase/auth';
    ...
    if (this.state.hasError) return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4" dir="rtl">
        <div className="bg-white p-8 rounded-3xl shadow-xl text-center max-w-md w-full border border-rose-100">
          ...
          <h2 className="text-xl font-black text-slate-800 mb-2">حدث خطأ غير متوقع</h2>
          <p className="text-slate-500 text-sm mb-6">{this.state.error?.message}</p>
          <button onClick={() => window.location.reload()} className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold w-full">تحديث الصفحة</button>
        </div>
      </div>
    ); 
...
const CustomModal = ({ title, onClose, children }) => (
  <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm transition-opacity" dir="rtl">
    <div className="bg-white dark:bg-slate-800 rounded-3xl w-full max-w-md max-h-[90vh] overflow-y-auto p-6 shadow-2xl relative custom-scrollbar border border-slate-200 dark:border-slate-700 animate-in fade-in zoom-in duration-200">
      <div className="flex justify-between items-center mb-5 border-b border-slate-100 dark:border-slate-700 pb-3 sticky top-0 bg-white dark:bg-slate-800 z-10">
        <h2 className="text-xl font-black text-slate-800 dark:text-white">{title}</h2>
        <button onClick={onClose} className="p-2 bg-slate-100 dark:bg-slate-700 rounded-xl text-slate-500 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-900/30 transition-colors"><X size={18} /></button>
      </div>
      {children}
    </div>
  </div>
...
// --- PDF EXPORT HELPERS (Requires jspdf & jspdf-autotable) ---
const exportReportPDF = async (title, data, period, cafeName) => {
  try {
    const { default: jsPDF } = await import('jspdf');
    await import('jspdf-autotable');
    const doc = new jsPDF('p', 'pt', 'a4', true);
    doc.addFont("https://fonts.gstatic.com/ea/cairo/v4/Cairo-Regular.ttf", "Cairo", "normal");
    doc.setFont("Cairo");
    doc.setLanguage("ar-EG");
    doc.setFontSize(20);
    doc.text(`تقرير المبيعات - ${cafeName}`, 300, 40, { align: 'center' });
    doc.setFontSize(12);
    doc.text(`الفترة: ${period} | المبيعات: ${data.totalRevenue} | المصروفات: ${data.totalExpenses} | الأرباح: ${data.netProfit}`, 300, 65, { align: 'center' });
    
    const tableData = data.orders.map(o => [o.total.toFixed(2), o.discountAmount > 0 ? o.discountAmount : '-', (o.items || []).map(i => i.name).join('، '), o.note || 'تيك أواي', o.cashierName || '-', o.date]);
    doc.autoTable({
      head: [['الإجمالي', 'الخصم', 'الأصناف', 'النوع', 'الكاشير', 'التاريخ']],
      body: tableData,
      startY: 85,
    ...
    toast.error('حدث خطأ أثناء تصدير PDF. تأكد من تثبيت مكتبة jspdf.');
  }
};

const exportEmployeeReportPDF = async (emp, orders, cafeName) => {
  try {
    const { default: jsPDF } = await import('jspdf');
    await import('jspdf-autotable');
    const doc = new jsPDF('p', 'pt', 'a4', true);
    doc.addFont("https://fonts.gstatic.com/ea/cairo/v4/Cairo-Regular.ttf", "Cairo", "normal");
    doc.setFont("Cairo");
    doc.text(`تقرير الموظف: ${emp.name} - ${cafeName}`, 300, 40, { align: 'center' });
    const empOrders = orders.filter(o => o.cashierName === emp.name);
    const tableData = empOrders.map(o => [o.total.toFixed(2), (o.items || []).map(i => i.name).join('، '), o.date]);
    doc.autoTable({
      head: [['الإجمالي', 'الأصناف', 'التاريخ']],
      body: tableData,
      startY: 70,
      styles: { font: 'Cairo', halign: 'right' },
      headStyles: { fillColor: [79, 70, 229] }
    });
    doc.save(`Employee_${emp.name}_${Date.now()}.pdf`);
    toast.success('تم تحميل التقرير بنجاح');
  } catch (e) {
    console.error("PDF Export Error:", e);
...
const AuthHandler = ({ onLoginSuccess }) => {
  const [status, setStatus] = useState('جاري التحقق من الرابط...');
  const navigate = useNavigate();
  useEffect(() => {
    const handleEmailLink = async () => {
      if (isSignInWithEmailLink(auth, window.location.href)) {
        let email = window.localStorage.getItem('emailForSignIn');
        if (!email) email = window.prompt('الرجاء إدخال بريدك الإلكتروني لتأكيد الدخول');
        try {
          const result = await signInWithEmailLink(auth, email, window.location.href);
          window.localStorage.removeItem('emailForSignIn');
          setStatus('تم تسجيل الدخول بنجاح! جاري التوجيه...');
          if (onLoginSuccess) await onLoginSuccess(result.user);
          navigate('/');
        } catch (error) {
  ...
  return <div className="min-h-screen flex items-center justify-center bg-slate-50"><div className="bg-white p-8 rounded-2xl shadow-lg font-black text-indigo-600 text-lg flex items-center gap-3"><Loader2 className="animate-spin"/> {status}</div></div>;
};
...
function appReducer(state, action) {
  switch (action.type) {
    ...
    case 'RESET_CAFE_DATA':
      return {
        ...state, rawMaterials: [], products: defaultProducts, employees: [], expenses: [],
        tables: [], shifts: [], orders: [], activeTableOrders: {}, isTaxEnabled: false,
        offers: [], psDevices: [], psSessions: []
      };
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setField('isDataLoaded', true), 3000); // Fail-safe
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(platformRef, { includeMetadataChanges: true }, (snap) => {
      if (snap.exists()) {
        ...
        setState({ globalSettings: data.globalSettings || { appName: 'كوفي سحابة' }, tenants: data.tenants || [] });
      }
      setField('isDataLoaded', true);
  ...
  // Fetch Cafe Data
  useEffect(() => {
    if (!fbUser || !currentUser?.cafeId) return;
    ...
    }, (err) => { console.error("Cafe data error:", err); toast.error('فشل تحميل بيانات الكافيه'); });
    return () => unsub();
  }, [fbUser, currentUser?.cafeId, setField, setState]);

  // Global Sync Functions
  const syncPlatformToCloud = useCallback(async (newData) => {
    if (!fbUserRef.current) throw new Error("غير متصل");
    try {
  ...
  const syncToCloud = useCallback(async (newData) => {
    ...
    if (!fbUserRef.current || !cafeId) return;
    ...
    try {
      ...
      const msg = err.code === 'permission-denied' ? 'صلاحية غير كافية' : err.code === 'unavailable' ? 'غير متصل بالإنترنت' : err.message;
  ...
  const processUserRole = useCallback(async (user) => {
    ...
    if (email === OWNER_EMAIL) {
      ...
      toast.success('مرحباً بمالك المنصة');
      return true;
    }
  ...
  const expiredProducts = useMemo(() => { const today = new Date(); return products.filter(p => p.expiryDate && new Date(p.expiryDate) <= today); }, [products]);
  ...
  const activeShift = useMemo(() => {
    ...
    return shifts.find(s => s.status === 'open' && s.cashierName?.trim() === currentUser.name?.trim());
  }, [shifts, currentUser]);
  ...
  const closeModal = useCallback(() => setState({ activeModal: null, formData: {}, deleteConfig: null }), [setState]);

  const handleFormChange = useCallback((e) => setState({ formData: { ...stateRef.current.formData, [e.target.name]: e.target.value } }), [setState]);
  ...
  const handleImageUpload = useCallback((e) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        setState({ formData: { ...stateRef.current.formData, image: canvas.toDataURL('image/jpeg', 0.6) } });
      };
  ...
  }, [deleteConfig, rawMaterials, products, employees, tables, expenses, offers, psDevices, setField, syncToCloud, closeModal]);
  ...
  }, [setField, syncToCloud, closeModal]);
  ...
  const saveTenant = useCallback(async (e) => {
    ...
    if (currentForm.isNew) {
      if (tenants.find(t => t.id === currentForm.id)) { toast.error('كود الكافيه موجود بالفعل!'); return; }
    ...
    try {
      ...
      closeModal();
      toast.success('تم حفظ بيانات الكافيه');
    } catch (error) {}
  }, [tenants, setField, syncPlatformToCloud, closeModal]);
  ...
  const startPsSession = useCallback((deviceId) => {
    ...
    toast.success(`تم بدء جلسة على ${device.name}`);
  }, [psDevices, psSessions, currentUser, setField, syncToCloud]);
  ...
  const endPsSession = useCallback((sessionId) => {
    ...
    syncToCloud(updates);
    toast.success(`تم إنهاء الجلسة — ${roundedMin} دقيقة (${cost.toFixed(2)} ج)`);
  }, [psSessions, psDevices, orders, activeShift, currentUser, setField, syncToCloud]);

  // ========== POS Logic ==========
  const processOrder = useCallback(() => {
    if (cart.length === 0) { toast.error('السلة فارغة'); return; }
    if (orderType === 'dine_in' && !activeTableId) { toast.error('يرجى تحديد الطاولة أولاً!'); return; }
    if (currentUser.role === 'cashier' && !activeShift) { toast.error('يجب استلام عهدة أولاً!'); return; }
    ...
    toast.success(`تم إتمام الطلب بقيمة ${total.toFixed(2)} ج`);
  }, [cart, orderType, activeTableId, currentUser, activeShift, discountValue, discountType, isTaxEnabled, rawMaterials, products, tables, orders, activeTableOrders, setState, syncToCloud]);
  ...
  const holdTableOrder = useCallback(async () => {
    ...
    try {
      ...
      toast.success('تم حفظ الطلب على الطاولة');
    } catch (error) { toast.error('فشل حفظ الطلب'); }
    finally { setField('isHoldingTable', false); }
  }, [isHoldingTable, cart, activeTableId, activeTableOrders, setField, setState, syncToCloud]);
  ...
  const navItems = [
    ...
    { id: 'offers', icon: <Tag size={19} />, label: 'العروض', roles: ['admin', 'cashier'] }, // ✅ متوفر للكاشير
    { id: 'tables', icon: <Utensils size={19} />, label: 'الصالة', roles: ['admin'] },
    { id: 'playstation', icon: <Gamepad2 size={19} />, label: 'بلايستيشن', roles: ['admin', 'cashier'] }, // ✅ متوفر للكاشير
      ...
      <div className={isDarkMode ? 'dark' : ''}>
          ...
          <div className="flex items-center gap-2">
            {currentUser && lowStockItems.length > 0 && (
              <button onClick={() => setField('currentRoute', 'inventory')} className="flex items-center gap-1 bg-white/20 px-2 py-0.5 rounded-full">
                <Bell size={11} /> {lowStockItems.length} تنبيه مخزون
              </button>
            )}
          </div>
        ...
        {!currentUser ? (
          <div dir="rtl" className="min-h-screen bg-slate-100 dark:bg-slate-900 flex items-center justify-center font-sans relative overflow-hidden p-4 pt-10">
            ...
            <div className="bg-white dark:bg-slate-800 p-8 md:p-10 rounded-3xl shadow-2xl w-full max-w-md border-t-8 border-indigo-600 z-10 relative animate-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex gap-2 mb-6 p-1 bg-slate-100 dark:bg-slate-700 rounded-xl">
                <button type="button" onClick={() => setField('loginMethod', 'password')} className={`flex-1 py-2.5 rounded-lg font-bold text-sm transition-colors ${loginMethod === 'password' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>كلمة المرور</button>
                <button type="button" onClick={() => setField('loginMethod', 'emailLink')} className={`flex-1 py-2.5 rounded-lg font-bold text-sm transition-colors ${loginMethod === 'emailLink' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600'}`}>رابط سريع</button>
              </div>
              ...
              {loginMethod === 'password' ? (
                <form onSubmit={handleLogin} className="space-y-4">
                  ...
                  <button type="submit" disabled={isLoggingIn} className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white font-bold py-4 rounded-2xl transition-all shadow-lg text-lg flex items-center justify-center gap-2">
                ...
                <form onSubmit={handleSendEmailLink} className="space-y-4">
                  <div className="relative">
                    <Mail className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5 pointer-events-none" />
                    <input required type="email" placeholder="البريد الإلكتروني" value={emailLinkEmail} onChange={e => { setField('emailLinkEmail', e.target.value); setField('emailLinkError', ''); }} className="w-full p-4 pr-12 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none focus:border-indigo-500 text-slate-800 dark:text-white font-bold text-sm" dir="ltr" />
                  </div>
                  <button type="submit" disabled={emailLinkLoading} className="w-full bg-indigo-600 hover:bg-indigo-700 disabled:opacity-60 text-white font-bold py-4 rounded-2xl transition-all shadow-lg text-lg flex items-center justify-center gap-2">
                    {emailLinkLoading ? <Loader2 size={20} className="animate-spin" /> : 'إرسال رابط الدخول'}
                  </button>
                </form>
              )}
            </div>
        ...
        // CUSTOMER MENU
          ...
          <div dir="rtl" className="min-h-screen bg-slate-50 dark:bg-slate-900 pb-10 pt-7">
            <header className="bg-white dark:bg-slate-800 p-4 px-8 shadow-sm sticky top-0 z-30 flex justify-between items-center border-b border-slate-200 dark:border-slate-700">
              <h1 className="text-2xl font-black text-indigo-600 flex items-center gap-2"><Coffee /> منيو {globalSettings.appName}</h1>
              <button onClick={() => setState({ currentUser: null })} className="text-xs font-bold bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-white px-4 py-2 rounded-lg hover:bg-indigo-600 hover:text-white transition-colors">خروج</button>
            </header>
            <div className="p-4 md:px-8 flex gap-2 overflow-x-auto no-scrollbar sticky top-[73px] bg-slate-50/95 dark:bg-slate-900/95 backdrop-blur-md z-20 pt-4 pb-3">
              <button onClick={() => setField('selectedCategoryFilter', 'all')} className={`px-6 py-2.5 rounded-full font-bold text-sm ${selectedCategoryFilter === 'all' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-white'}`}>الكل</button>
              {categories.map(c => (
                <button key={c.id} onClick={() => setField('selectedCategoryFilter', c.id)} className={`px-6 py-2.5 rounded-full font-bold text-sm ${selectedCategoryFilter === c.id ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-white'}`}>{c.name}</button>
              ))}
            </div>
            <div className="p-4 md:px-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 max-w-7xl mx-auto">
              {(selectedCategoryFilter === 'all' ? products : products.filter(p => p.category === selectedCategoryFilter)).map(p => {
                ...
                return (
                  <div key={p.id} className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700">
                    {hasOffer && <div className="absolute top-3 left-3 bg-rose-500 text-white text-[10px] font-black px-2 py-1 rounded-full z-10">عرض!</div>}
                    {p.image ? <img src={p.image} alt={p.name} className="w-full h-48 object-cover" /> : <div className="w-full h-48 bg-indigo-50 dark:bg-indigo-900 flex items-center justify-center text-indigo-300"><Coffee size={50}/></div>}
                    <div className="p-4">
                      <h3 className="font-black text-lg text-slate-800 dark:text-white">{p.name}</h3>
                      <div className="flex items-center gap-2 mt-2">
                        {hasOffer && <span className="text-slate-400 line-through text-sm">{p.price} ج</span>}
                        <span className={`font-black text-xl ${hasOffer ? 'text-rose-500' : 'text-indigo-600'}`}>{op.toFixed(0)} ج.م</span>
                      </div>
                    </div>
                  </div>
                );
        ...
        // ADMIN / CASHIER / SUPER_ADMIN VIEWS
        ) : (
                ...
                <div className={`fixed inset-y-0 right-0 z-50 transform ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"} lg:relative lg:translate-x-0 transition-transform duration-300 w-64 md:w-72 bg-white dark:bg-slate-900 flex flex-col shrink-0 pt-7 border-l border-slate-200 dark:border-slate-800 shadow-xl lg:shadow-none`}>
                  <div className="p-4 md:p-6 border-b border-slate-100 dark:border-slate-800 shrink-0 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-indigo-600 dark:text-indigo-400 text-xs mt-1 font-bold">{currentUser.cafeName}</p>
                    </div>
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative w-full">
              ...
              <header className="p-3 md:p-4 md:px-8 flex justify-between items-center shadow-sm z-30 shrink-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                <div className="flex items-center gap-2">
                  ...
                  <h1 className="font-black text-sm md:text-xl text-slate-800 dark:text-white">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  {/* Force Refresh Button (Fix for caching/deployment issues) */}
              ...
              <div className="flex-1 overflow-auto custom-scrollbar relative">
                ...
                {currentUser.role === 'cashier' && !activeShift && activeModal !== 'closeShift' ? (
                  <div className="absolute inset-0 z-30 flex items-center justify-center bg-slate-100 dark:bg-slate-900 p-4">
                    <div className="bg-white dark:bg-slate-800 p-6 md:p-10 rounded-3xl shadow-2xl max-w-md w-full text-center border border-slate-200 dark:border-slate-700 animate-in zoom-in-95 duration-300">
                      ...
                      {shifts.filter(s => s.status === 'open').length > 0 && (
                        <div className="mb-5 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-xl border border-amber-200 dark:border-amber-800 text-right">
                          ...
                          {shifts.filter(s => s.status === 'open').map(s => <p key={s.id} className="text-xs text-amber-600 font-bold">• {s.cashierName} — {s.startTime}</p>)}
                        </div>
                      )}
                      <form onSubmit={(e) => {
                        e.preventDefault();
                        const newShift = {
                          id: 'sh_' + Date.now(), cashierName: currentUser.name.trim(), cafeId: currentUser.cafeId,
                          startTime: new Date().toLocaleString('ar-EG'), timestamp: Date.now(),
                          startingCash: parseFloat(e.target.startingCash.value) || 0, status: 'open'
                        };
                        const newShifts = [...shifts, newShift];
                        setField('shifts', newShifts); syncToCloud({ shifts: newShifts });
                        ...
                        <div className="text-right mb-5">
                          ...
                          <input required name="startingCash" type="number" min="0" step="any" placeholder="0.00" className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-2xl text-center font-black text-2xl outline-none focus:border-indigo-500 text-slate-800 dark:text-white" />
                        </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
                      ...
                      <div className="p-6">
                        <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-200 dark:border-amber-800 space-y-2">
                          <p className="font-black text-amber-800 dark:text-amber-300 text-sm">📋 خطوات الرفع (JSON):</p>
                          <p className="text-xs font-bold text-amber-700 dark:text-amber-400">١. اختر الكافيه المستهدف ثم الصق بيانات المواد الخام (اختياري)</p>
                          <p className="text-xs font-bold text-amber-700 dark:text-amber-400">٢. الصق بيانات المنتجات (مع الريسيبي) ثم اضغط رفع</p>
                        </div>
                        {(() => {
                          ...
                          const handleUpload = async () => {
                            ...
                            if (!uploadMaterials && !uploadProducts) { setState({ formData: { ...stateRef.current.formData, _uploadError: 'يجب إدخال مواد أو منتجات' } }); return; }
                            setState({ formData: { ...stateRef.current.formData, _uploadStatus: 'uploading', _uploadError: '' } });

                            try {
                              let parsedMaterials = null; let parsedProducts = null;
                              if (uploadMaterials.trim()) {
                                parsedMaterials = JSON.parse(uploadMaterials.trim());
                                parsedMaterials.forEach((rm, i) => { if (!rm.id) rm.id = `rm_${Date.now()}_${i}`; if (!rm.unit) rm.unit = 'جرام'; });
                              }
                              if (uploadProducts.trim()) {
                                parsedProducts = JSON.parse(uploadProducts.trim());
                                parsedProducts.forEach((p, i) => { if (!p.id) p.id = `prod_${Date.now()}_${i}`; if (!p.recipe) p.recipe = []; });
                              }
                              ...
                              setState({ formData: { ...stateRef.current.formData, _uploadStatus: 'success', _uploadError: '', _uploadMaterials: '', _uploadProducts: '' }});
                              toast.success('تم رفع المنيو بنجاح');
                            } catch (err) { setState({ formData: { ...stateRef.current.formData, _uploadStatus: 'error', _uploadError: err.message } }); }
                          };
                          ...
                          return (
                            <div className="space-y-5">
                              <div>
                                ...
                                <select value={uploadCafeId} onChange={e => setState({ formData: { ...stateRef.current.formData, _uploadCafeId: e.target.value, _uploadStatus: '', _uploadError: '' } })} className="w-full p-3 bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-xl font-bold text-slate-800 dark:text-white outline-none focus:border-indigo-500">
                                  <option value="">— اختر كافيه —</option>
                                  {tenants.filter(t => t.status === 'active').map(t => <option key={t.id} value={t.id}>{t.name} ({t.id})</option>)}
                                </select>
                              </div>
                              <div>
                                <label className="block text-sm font-black mb-2 text-slate-700 dark:text-slate-300 flex items-center gap-2"><Package size={15} className="text-indigo-500" /> المواد الخام (JSON)</label>
                                <textarea value={uploadMaterials} onChange={e => setState({ formData: { ...stateRef.current.formData, _uploadMaterials: e.target.value, _uploadStatus: '', _uploadError: '' } })} rows={3} className="w-full p-3 bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-xl font-mono text-xs outline-none focus:border-indigo-500" dir="ltr" />
                              </div>
                              <div>
                                <label className="block text-sm font-black mb-2 text-slate-700 dark:text-slate-300 flex items-center gap-2"><Coffee size={15} className="text-indigo-500" /> المنتجات (JSON)</label>
                                <textarea value={uploadProducts} onChange={e => setState({ formData: { ...stateRef.current.formData, _uploadProducts: e.target.value, _uploadStatus: '', _uploadError: '' } })} rows={3} className="w-full p-3 bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-xl font-mono text-xs outline-none focus:border-indigo-500" dir="ltr" />
                              </div>
                              {uploadError && <div className="p-4 bg-rose-50 text-rose-700 rounded-xl font-bold">{uploadError}</div>}
                              <button onClick={handleUpload} disabled={uploadStatus === 'uploading' || !uploadCafeId} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white py-4 rounded-2xl font-black text-lg disabled:opacity-50">
                                {uploadStatus === 'uploading' ? <><Loader2 size={20} className="animate-spin inline" /> جاري الرفع...</> : <><Upload size={20} className="inline" /> رفع</>}
                              </button>
                            </div>
                          );
                        })()}
                ...
                // --- POS (Point of Sale) ---
                  ...
                  <div className="flex flex-col lg:flex-row h-full p-2 md:p-4 lg:p-6 gap-4 lg:gap-6 overflow-hidden relative animate-in fade-in duration-300">
                    <div className="flex-1 flex flex-col gap-4 overflow-hidden pb-16 lg:pb-0">
                      ...
                      {orderType === 'dine_in' && !activeTableId && (
                        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-3 animate-in slide-in-from-top-4">
                          {tables.map(t => {
                        ...
                        {(selectedCategoryFilter === 'all' ? products : products.filter(p => p.category === selectedCategoryFilter)).map(p => {
                          ...
                          return (
                            <button key={p.id} onClick={() => {
                              if (orderType === 'dine_in' && !activeTableId) { toast.error('الرجاء اختيار طاولة'); return; }
                              ...
                              {p.image ? <img src={p.image} alt={p.name} className="w-16 h-16 rounded-full object-cover shadow-sm group-hover:scale-110 transition-transform border-2 border-indigo-50 dark:border-slate-700" /> : <div className="w-16 h-16 bg-indigo-50 dark:bg-indigo-900/40 text-indigo-600 rounded-full flex items-center justify-center"><Coffee className="w-7 h-7" /></div>}
                    ...
                    <div className={`w-full lg:w-[350px] xl:w-[420px] bg-white dark:bg-slate-800 rounded-t-3xl lg:rounded-3xl shadow-2xl lg:shadow-md border border-slate-200 dark:border-slate-700 flex flex-col h-[85vh] lg:h-full fixed lg:relative bottom-0 left-0 right-0 z-40 transition-transform duration-300 ${isMobileCartOpen ? 'translate-y-0' : 'translate-y-full lg:translate-y-0'}`}>
                      ...
                      <div className="flex-1 overflow-auto p-4 space-y-2 custom-scrollbar">
                        {cart.length === 0 ? (
                          ...
                          <div key={item.id} className="bg-slate-50 dark:bg-slate-700/50 p-2.5 rounded-2xl border border-slate-100 dark:border-slate-600 flex justify-between items-center animate-in fade-in">
                            <div className="flex gap-2.5 items-center">
                              {item.image ? <img src={item.image} alt={item.name} className="w-10 h-10 rounded-xl object-cover shadow-sm" /> : <div className="w-10 h-10 bg-indigo-100 dark:bg-indigo-900 text-indigo-500 rounded-xl flex items-center justify-center"><Coffee size={14} /></div>}
                        ...
                        {currentUser.role === 'admin' && cart.length > 0 && (
                          <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-200 dark:border-amber-800">
                            ...
                            <div className="flex gap-2">
                              ...
                              <input type="number" min="0" step="any" value={discountValue} onChange={e => setField('discountValue', e.target.value)} placeholder={discountType === 'percent' ? 'نسبة %' : 'مبلغ'} className="flex-1 p-2 bg-white dark:bg-slate-800 border border-amber-200 dark:border-amber-800 rounded-xl text-xs font-bold outline-none focus:border-amber-500 text-center" />
                        ...
                        <table className="w-full text-right min-w-[650px]">
                          <thead className="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700 font-bold text-slate-500 text-sm">
                            <tr>
                              ...
                              <th className="p-4 text-center">تعديل مباشر</th>
                          ...
                          <tbody>
                            {rawMaterials.map(rm => (
                              <tr key={rm.id} className={`border-b border-slate-100 dark:border-slate-700 text-sm ${rm.currentStock <= STOCK_ALERT_THRESHOLD ? 'bg-rose-50/50 dark:bg-rose-900/10' : ''}`}>
                                <td className="p-4 font-black text-slate-800 dark:text-white"><div className="flex items-center gap-2">{rm.currentStock <= STOCK_ALERT_THRESHOLD && <Bell size={13} className="text-rose-500 shrink-0" />}{rm.name}</div></td>
                                <td className="p-4 text-slate-500 dark:text-slate-400 font-bold">{rm.unit}</td>
                                <td className="p-4 text-center"><span className={`px-3 py-1.5 rounded-xl font-black text-xs ${rm.currentStock <= 0 ? 'bg-red-100 text-red-700' : rm.currentStock <= STOCK_ALERT_THRESHOLD ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'}`}>{rm.currentStock}</span></td>
                                <td className="p-2 text-center">
                                  <div className="flex items-center justify-center gap-1">
                                    <button onClick={() => { const v = Math.max(0, (rm.currentStock || 0) - 1); const arr = rawMaterials.map(r => r.id === rm.id ? { ...r, currentStock: v } : r); setField('rawMaterials', arr); syncToCloud({ rawMaterials: arr }); }} className="w-8 h-8 rounded-lg bg-rose-50 text-rose-500 hover:bg-rose-500 hover:text-white font-black">−</button>
                                    <div className="w-20"><InlineSafeInput value={rm.currentStock} onSave={v => { const arr = rawMaterials.map(r => r.id === rm.id ? { ...r, currentStock: Math.max(0, v) } : r); setField('rawMaterials', arr); syncToCloud({ rawMaterials: arr }); }} colorClass="text-indigo-600 focus:border-indigo-500 border-slate-200 bg-slate-50 dark:bg-slate-900 dark:border-slate-700" /></div>
                                    <button onClick={() => { const v = (rm.currentStock || 0) + 1; const arr = rawMaterials.map(r => r.id === rm.id ? { ...r, currentStock: v } : r); setField('rawMaterials', arr); syncToCloud({ rawMaterials: arr }); }} className="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-500 hover:text-white font-black">+</button>
                                  </div>
                                </td>
                                <td className="p-2 text-center"><InlineSafeInput value={rm.costPerUnit} onSave={v => { const arr = rawMaterials.map(r => r.id === rm.id ? { ...r, costPerUnit: v } : r); setField('rawMaterials', arr); syncToCloud({ rawMaterials: arr }); }} colorClass="text-indigo-600 focus:border-indigo-500 border-indigo-100 bg-indigo-50 dark:bg-indigo-900/20" /></td>
                                <td className="p-4 text-center"><button onClick={() => setState({ deleteConfig: { type: 'material', id: rm.id }, activeModal: 'delete' })} className="text-rose-500 p-2 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-xl"><Trash2 size={15} /></button></td>
                              </tr>
                            ))}
                            {rawMaterials.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-slate-400 font-bold">لا توجد مواد خام مضافة بعد</td></tr>}
                          </tbody>
                        </table>
                      </div>
                    </div>
                          ...
                          <div key={p.id} className={`bg-white dark:bg-slate-800 p-5 rounded-3xl border-2 shadow-sm ${isExp ? 'border-red-300 dark:border-red-700' : isNear ? 'border-amber-300 dark:border-amber-700' : 'border-slate-200 dark:border-slate-700'}`}>
                            <div className="flex justify-between mb-4">
                              <div className="flex items-center gap-3">
                                {p.image ? <img src={p.image} alt={p.name} className="w-12 h-12 rounded-xl object-cover shadow-sm" /> : <div className="w-12 h-12 rounded-xl bg-indigo-50 text-indigo-500 flex items-center justify-center"><Coffee size={19} /></div>}
                            ...
                            <button onClick={() => setState({ deleteConfig: { type: 'product', id: p.id }, activeModal: 'delete' })} className="text-rose-500 bg-rose-50 hover:bg-rose-100 p-2.5 rounded-xl"><Trash2 size={15} /></button>
                          </div>
                        );
                      })}
                    </div>
                ...
                // --- OFFERS ---
                  ...
                  <div className="p-4 md:p-8 max-w-6xl mx-auto animate-in fade-in duration-300">
                    ...
                    {(() => {
                      ...
                      if (visibleOffers.length === 0) return <div className="text-center py-20 text-slate-400"><Gift className="w-20 h-20 mx-auto mb-4 opacity-20" /><p className="font-bold text-lg">لا توجد عروض فعالة</p></div>;
                      return (
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                      ...
                      {psDevices.length === 0 ? (
                        ...
                        return (
                          <div key={device.id} className={`bg-white dark:bg-slate-800 p-5 rounded-3xl border-2 shadow-sm ${aS ? 'border-emerald-400 dark:border-emerald-600' : 'border-slate-200 dark:border-slate-700'}`}>
                            ...
                            {currentUser.role === 'admin' && (
                              <button onClick={() => setState({ deleteConfig: { type: 'psDevice', id: device.id }, activeModal: 'delete' })} className="mt-3 w-full text-xs text-rose-400 hover:text-rose-600 font-bold py-1">حذف الجهاز</button>
                            )}
                          </div>
                        );
                      })}
                ...
                // --- HR ---
                  ...
                  <div className="p-4 md:p-8 max-w-7xl mx-auto animate-in fade-in duration-300">
                          ...
                          <tbody>
                            {employees.map(emp => (
                              <tr key={emp.id} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 text-sm">
                                ...
                                <td className="p-2"><InlineSafeInput value={emp.advances} onSave={(v) => genericSave('employees', employees, { advances: v, id: emp.id })} colorClass="text-amber-500 focus:border-amber-500 border-amber-100 bg-amber-50 dark:bg-amber-900/20" /></td>
                                <td className="p-2"><InlineSafeInput value={emp.deductions} onSave={(v) => genericSave('employees', employees, { deductions: v, id: emp.id })} colorClass="text-rose-500 focus:border-rose-500 border-rose-100 bg-rose-50 dark:bg-rose-900/20" /></td>
                                ...
                                <td className="p-5 text-center"><button onClick={() => setState({ deleteConfig: { type: 'employee', id: emp.id }, activeModal: 'delete' })} className="text-rose-500 p-2 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-xl"><Trash2 size={15} /></button></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                    ...
                    {selectedEmployeeReport && (() => {
                      ...
                      return (
                        <div className="bg-white dark:bg-slate-800 rounded-3xl border border-indigo-200 dark:border-indigo-800 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-4">
                          ...
                          <tbody>
                            {expenses.map(ex => (
                              <tr key={ex.id} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 text-sm">
                                ...
                                <td className="p-5 text-center"><button onClick={() => setState({ deleteConfig: { type: 'expense', id: ex.id }, activeModal: 'delete' })} className="text-rose-500 p-2 hover:bg-rose-50 dark:hover:bg-rose-900/30 rounded-xl"><Trash2 size={15} /></button></td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
            ...
            {activeModal === 'closeShift' && activeShift && (
              <CustomModal title="تقفيل الوردية" onClose={closeModal}>
                <form onSubmit={(e) => {
                  ...
                  setField('shifts', newShifts); syncToCloud({ shifts: newShifts });
                  closeModal(); handleLogout(); toast.success('تم تقفيل الوردية بنجاح');
                }}>

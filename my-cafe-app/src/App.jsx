    ...
    if (this.state.hasError) return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4" dir="rtl">
        <div className="bg-white p-8 rounded-3xl shadow-xl text-center max-w-md w-full border border-rose-100">
          ...
          <p className="text-slate-500 text-sm mb-6">{this.state.error?.message || "خطأ في تشغيل التطبيق"}</p>
...
const AuthHandler = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="bg-white p-8 rounded-2xl shadow-lg font-black text-indigo-600 text-lg flex items-center gap-3">
...
function MainApp() {
  ...
  const syncPlatformToCloud = useCallback(async (newData) => {
    if (!db || !fbUserRef.current) throw new Error("غير متصل");
    try {
  ...
  if (window.location.pathname === '/auth/email-link') {
    return <AuthHandler onLoginSuccess={handleAuthSuccess} />;
  }

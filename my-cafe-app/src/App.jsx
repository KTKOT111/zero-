...
import { 
  ...
  Bell, Tag, Gift, Gamepad2, Download, Calendar, AlertTriangle, User, Mail, Lock
} from 'lucide-react';
  ...
  render() {
    if (this.state.hasError) return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 p-6 text-center" dir="rtl">
        ...
        <pre className="bg-white p-4 rounded-xl shadow border border-rose-100 text-left text-sm overflow-auto max-w-2xl w-full text-rose-600" dir="ltr">{String(this.state.error?.message || this.state.error)}</pre>
...
// كل كافيه له إيميلات بالشكل: admin.CAFEID@coffeeerp.app و cashier.CAFEID@coffeeerp.app
// مالك المنصة: owner@coffeeerp.app
...
function exportReportPDF(title, data, period, cafeName) {
  ...
  const rows = data.orders.map((o,i) => `<tr style="border-bottom:1px solid #e2e8f0;${i%2===0?'background:#f8fafc':''}"><td style="padding:8px;text-align:right">${o.date||''}</td><td style="padding:8px;text-align:right">${o.note||'تيك أواي'}</td><td style="padding:8px;text-align:right">${(o.items||[]).map(it=>`${it.name}(${it.quantity})`).join('، ')}</td><td style="padding:8px;text-align:center">${o.discountAmount>0?`-${o.discountAmount.toFixed(2)}`:'-'}</td><td style="padding:8px;text-align:center;font-weight:bold;color:#4f46e5">${o.total.toFixed(2)} ج</td></tr>`).join('');
  const html = `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;margin:20px;color:#1e293b;direction:rtl}h1{color:#4f46e5}.stats{display:flex;gap:16px;margin-bottom:24px;flex-wrap:wrap}.stat{background:#f1f5f9;border-radius:12px;padding:12px 20px;min-width:140px;text-align:center}.stat-val{font-size:22px;font-weight:bold;color:#4f46e5}.stat-lbl{font-size:12px;color:#64748b}table{width:100%;border-collapse:collapse;font-size:13px}th{background:#4f46e5;color:white;padding:10px 8px;text-align:right}</style></head><body>
  <h1>تقرير المبيعات — ${periodLabels[period]||period}</h1><div style="color:#64748b;font-size:13px;margin-bottom:20px">${cafeName} | ${now}</div>
  <div class="stats"><div class="stat"><div class="stat-val">${data.totalRevenue.toFixed(2)} ج</div><div class="stat-lbl">إجمالي المبيعات</div></div><div class="stat"><div class="stat-val">${data.ordersCount}</div><div class="stat-lbl">عدد الطلبات</div></div><div class="stat"><div class="stat-val">${data.totalExpenses.toFixed(2)} ج</div><div class="stat-lbl">المصروفات</div></div><div class="stat"><div class="stat-val" style="color:${data.netProfit>=0?'#16a34a':'#dc2626'}">${data.netProfit.toFixed(2)} ج</div><div class="stat-lbl">صافي الربح</div></div></div>
...
function exportEmployeeReportPDF(employee, orders, cafeName) {
  ...
  const now = new Date().toLocaleDateString('ar-EG');
  const rows = empOrders.map((o,i) => `<tr style="border-bottom:1px solid #e2e8f0;${i%2===0?'background:#f8fafc':''}"><td style="padding:8px;text-align:right">${o.date||''}</td><td style="padding:8px;text-align:right">${(o.items||[]).map(it=>it.name).join('، ')}</td><td style="padding:8px;text-align:center;font-weight:bold;color:#4f46e5">${o.total.toFixed(2)} ج</td></tr>`).join('');
  const html = `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><style>body{font-family:Arial,sans-serif;margin:20px;direction:rtl}h1{color:#4f46e5}table{width:100%;border-collapse:collapse;font-size:13px}th{background:#4f46e5;color:white;padding:10px 8px;text-align:right}</style></head><body>
  <h1>تقرير معاملات: ${employee.name}</h1><p style="color:#64748b">${cafeName} | ${now}</p>
  <div style="display:flex;gap:16px;margin:16px 0"><div style="background:#f1f5f9;padding:12px 20px;border-radius:12px;text-align:center"><div style="font-size:20px;font-weight:bold;color:#4f46e5">${totalSales.toFixed(2)} ج</div><div style="font-size:12px;color:#64748b">إجمالي مبيعاته</div></div><div style="background:#f1f5f9;padding:12px 20px;border-radius:12px;text-align:center"><div style="font-size:20px;font-weight:bold;color:#4f46e5">${empOrders.length}</div><div style="font-size:12px;color:#64748b">عدد الطلبات</div></div><div style="background:#f1f5f9;padding:12px 20px;border-radius:12px;text-align:center"><div style="font-size:20px;font-weight:bold;color:#059669">${employee.salary} ج</div><div style="font-size:12px;color:#64748b">الراتب الأساسي</div></div></div>
...
export default function App() {
  ...
  const [tenants, setTenants]                     = useState([{ id: 'c1', name: 'كوفي سكول - فرع 1', status: 'active', subscriptionEnds: '2026-12-31', adminEmail: 'admin.c1@coffeeerp.app', cashierEmail: 'cashier.c1@coffeeerp.app' }]);
  ...
  const lowStockItems = useMemo(() => rawMaterials.filter(rm => rm.currentStock <= stockAlertThreshold), [rawMaterials]);
  const expiredProducts = useMemo(() => { const t=new Date(); return products.filter(p => p.expiryDate && new Date(p.expiryDate) <= t); }, [products]);
  const nearExpiryProducts = useMemo(() => { const t=new Date(), s=new Date(t.getTime()+7*24*60*60*1000); return products.filter(p => p.expiryDate && new Date(p.expiryDate) > t && new Date(p.expiryDate) <= s); }, [products]);

  useEffect(() => { fbUserRef.current = fbUser; }, [fbUser]);
  useEffect(() => { currentUserRef.current = currentUser; }, [currentUser]);
  const categories = useMemo(() => Array.from(new Set(products.map(p=>p.category).filter(Boolean))).map(c=>({id:c,name:c})), [products]);
  ...
  // Firebase Auth init (anonymous for data sync only)
  useEffect(() => {
    if(!auth||!db){setIsDataLoaded(true);return;}
    let isMounted=true, unsubAuth=null;
    const init = async () => {
      try {
        if(typeof __initial_auth_token!=='undefined'&&__initial_auth_token) { try{await signInWithCustomToken(auth,__initial_auth_token);}catch{await signInAnonymously(auth);} }
        else await signInAnonymously(auth);
      } catch(e){console.error("Auth error:",e);}
      if(isMounted){
        unsubAuth=onAuthStateChanged(auth,(user)=>{setFbUser(user);setIsDataLoaded(true);});
  ...
  useEffect(() => {
    ...
    const unsub=onSnapshot(ref,{includeMetadataChanges:true},(snap)=>{
      if(snap.exists()){const d=snap.data();if(d.globalSettings)setGlobalSettings(d.globalSettings);if(d.tenants)setTenants(d.tenants);}
      setIsDataLoaded(true);
    },(e)=>{console.error("Platform Error:",e);setIsDataLoaded(true);});
    return()=>unsub();
  }, [fbUser]);

  useEffect(() => {
    if(!fbUser||!db||!currentUser?.cafeId)return;
    setRawMaterials([]); setProducts(defaultProducts); setEmployees([]); setExpenses([]); setTables([]); setShifts([]); setOrders([]); setActiveTableOrders({}); setIsTaxEnabled(false); setOffers([]); setPsDevices([]); setPsSessions([]);
    const ref=doc(db,'coffee_erp_cafes',currentUser.cafeId);
    const unsub=onSnapshot(ref,{includeMetadataChanges:true},(snap)=>{
      if(snap.exists()){
        const d=snap.data();
        if(d.rawMaterials)setRawMaterials(d.rawMaterials);
        ...
        if(d.activeTableOrders)setActiveTableOrders(d.activeTableOrders);
        if(d.isTaxEnabled!==undefined)setIsTaxEnabled(d.isTaxEnabled);
        if(d.offers)setOffers(d.offers);
        if(d.psDevices)setPsDevices(d.psDevices);
        if(d.psSessions)setPsSessions(d.psSessions);
      }
      setIsSyncing(snap.metadata.hasPendingWrites);
    },(e)=>console.error("Cafe Error:",e));
    return()=>unsub();
  }, [fbUser, currentUser?.cafeId]);

  const syncPlatformToCloud = useCallback(async (newData) => {
    if(!db||!fbUserRef.current)return;
    try{await setDoc(doc(db,'coffee_erp_platform','config'),{...newData,lastUpdated:new Date().toISOString()},{merge:true});}
    catch(e){console.error("Platform sync error:",e);}
  }, []);
  ...
  const syncToCloud = useCallback(async (newData) => {
    ...
    try{
      ...
      setSyncStatus('success');setSyncError('');setTimeout(()=>setSyncStatus('idle'),2000);
    }catch(err){
      setSyncStatus('error');
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if(email === OWNER_EMAIL) {
        // التحقق من كلمة المرور محلياً (في الإنتاج يكون عبر Firebase Auth)
        // هنا نستخدم Firebase Email Auth
      ...
      // البحث عن الكافيه بالإيميل
      let matchedTenant = null, matchedRole = null;
      for(const t of tenants){
        if(t.adminEmail && t.adminEmail.toLowerCase() === email){ matchedTenant=t; matchedRole='admin'; break; }
        if(t.cashierEmail && t.cashierEmail.toLowerCase() === email){ matchedTenant=t; matchedRole='cashier'; break; }
      }

      if(!matchedTenant){ setLoginError('البريد الإلكتروني غير مسجل في النظام.'); return; }
      if(matchedTenant.status !== 'active'){ setLoginError('اشتراك الكافيه موقوف. يرجى التواصل مع الدعم.'); return; }

      await signInWithEmailAndPassword(auth, email, loginPassword);

      const displayName = matchedRole==='admin' ? `مدير - ${matchedTenant.name}` : `كاشير - ${matchedTenant.name}`;
      setCurrentUser({ name: displayName, role: matchedRole, cafeId: matchedTenant.id, cafeName: matchedTenant.name });
      setCurrentRoute(matchedRole==='admin' ? 'dashboard' : 'pos');
    } catch(err) {
      if(err.code==='auth/wrong-password'||err.code==='auth/invalid-credential') setLoginError('كلمة المرور غير صحيحة.');
      else if(err.code==='auth/user-not-found') setLoginError('البريد الإلكتروني غير مسجل في Firebase.');
      else if(err.code==='auth/too-many-requests') setLoginError('محاولات كثيرة. يرجى الانتظار قبل المحاولة مجدداً.');
      else setLoginError(`خطأ: ${err.message}`);
    } finally { setIsLoggingIn(false); }
  };

  const handleLogout = useCallback(() => {
    setCurrentUser(null); setLoginEmail(''); setLoginPassword('');
    setCart([]); setLastOrder(null);
  }, []);
  ...
  const getProductPriceWithOffer = useCallback((product) => {
    ...
    const offer=offers.find(o=>{
      ...
      const s=o.startDate?new Date(o.startDate):null, en=o.endDate?new Date(o.endDate):null;
  ...
  const processOrder = () => {
    ...
    if(currentUser.role==='admin'&&discountValue&&parseFloat(discountValue)>0){
      ...
      disc=discountType==='percent'?Math.min(sub,sub*dv/100):Math.min(sub,dv);
    }
    const after=sub-disc, tax=isTaxEnabled?after*taxRate:0, total=after+tax;
    const newRM=[...rawMaterials];
    cart.forEach(ci=>{const p=products.find(p=>p.id===ci.id);if(p?.recipe)p.recipe.forEach(r=>{const idx=newRM.findIndex(rm=>rm.id===r.materialId);if(idx!==-1)newRM[idx].currentStock-=(r.amount*ci.quantity);});});
    const newOrder={id:Date.now(),items:cart,subtotal:sub,discountAmount:disc,discountType:disc>0?discountType:null,discountValue:disc>0?parseFloat(discountValue):null,tax,total,date:new Date().toLocaleString('ar-EG'),timestamp:Date.now(),note:orderType==='takeaway'?'تيك أواي':`صالة - ${tables.find(t=>t.id===activeTableId)?.name}`,shiftId:activeShift?activeShift.id:null,cashierName:currentUser.name};
    const updOrders=[...orders,newOrder];
    let updATO={...activeTableOrders};
    if(orderType==='dine_in')delete updATO[activeTableId];
    setRawMaterials(newRM);setOrders(updOrders);setActiveTableOrders(updATO);
    syncToCloud({rawMaterials:newRM,orders:updOrders,activeTableOrders:updATO});
  ...
  const financialMetrics = useMemo(() => {
    const filter=(ts)=>{
      if(!ts||reportPeriod==='all')return true;
      const now=new Date(),d=new Date(ts);
      if(reportPeriod==='daily')return d.toDateString()===now.toDateString();
      if(reportPeriod==='weekly'){const s=new Date(now);s.setDate(now.getDate()-now.getDay());return d>=s;}
      if(reportPeriod==='monthly')return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();
      if(reportPeriod==='quarterly')return Math.floor(now.getMonth()/3)===Math.floor(d.getMonth()/3)&&d.getFullYear()===now.getFullYear();
      if(reportPeriod==='semi'){const h=Math.floor(now.getMonth()/6);return Math.floor(d.getMonth()/6)===h&&d.getFullYear()===now.getFullYear();}
      if(reportPeriod==='yearly')return d.getFullYear()===now.getFullYear();
      return true;
    };
    const fo=(orders||[]).filter(o=>filter(o.timestamp)), fe=(expenses||[]).filter(e=>filter(new Date(e.date).getTime()));
    const rev=fo.reduce((s,o)=>s+o.total,0), exp=fe.reduce((s,e)=>s+e.amount,0);
    let cogs=0;fo.forEach(o=>{(o.items||[]).forEach(it=>{const p=(products||[]).find(p=>p.id===it.id);if(p?.recipe)p.recipe.forEach(r=>{const rm=(rawMaterials||[]).find(rm=>rm.id===r.materialId);if(rm)cogs+=(r.amount*it.quantity*rm.costPerUnit);});});});
    return{totalRevenue:rev,totalExpenses:exp,totalCogs:cogs,netProfit:rev-(exp+cogs),orders:fo,ordersCount:fo.length};
  ...
  const confirmDelete=()=>{
    let upd={};
    if(deleteConfig.type==='material'){const u=rawMaterials.filter(rm=>rm.id!==deleteConfig.id);setRawMaterials(u);upd.rawMaterials=u;}
    if(deleteConfig.type==='product'){const u=products.filter(p=>p.id!==deleteConfig.id);setProducts(u);upd.products=u;}
    if(deleteConfig.type==='employee'){const u=employees.filter(e=>e.id!==deleteConfig.id);setEmployees(u);upd.employees=u;}
    if(deleteConfig.type==='table'){const u=tables.filter(t=>t.id!==deleteConfig.id);setTables(u);upd.tables=u;}
    if(deleteConfig.type==='expense'){const u=expenses.filter(ex=>ex.id!==deleteConfig.id);setExpenses(u);upd.expenses=u;}
    if(deleteConfig.type==='offer'){const u=offers.filter(o=>o.id!==deleteConfig.id);setOffers(u);upd.offers=u;}
    if(deleteConfig.type==='psDevice'){const u=psDevices.filter(d=>d.id!==deleteConfig.id);setPsDevices(u);upd.psDevices=u;}
    syncToCloud(upd);closeModal();
  };

  const genericSave=(col,arr,setter,extra={})=>{
    const upd=formData.id?arr.map(i=>i.id===formData.id?{...i,...formData,...extra}:i):[...arr,{...formData,...extra,id:`${col}_${Date.now()}`}];
    setter(upd);syncToCloud({[col]:upd});closeModal();
  };
  ...
  const saveTenant=(e)=>{
    ...
    if(formData.isNew){
      ...
      const upd=[...tenants,{...formData,status:'active'}];delete upd[upd.length-1].isNew;
      setTenants(upd);syncPlatformToCloud({tenants:upd});
    } else {
      const upd=tenants.map(t=>t.id===formData.id?{...t,...formData}:t);
      setTenants(upd);syncPlatformToCloud({tenants:upd});
    }
    closeModal();
  };
  ...
  const saveOffer=(e)=>{
    e.preventDefault();
    genericSave('offers',offers,setOffers,{name:formData.offerName,discountType:formData.offerDiscountType||'percent',discountValue:parseFloat(formData.offerDiscountValue)||0,productId:formData.offerProductId||null,category:formData.offerCategory||null,startDate:formData.offerStartDate||null,endDate:formData.offerEndDate||null,isActive:true});
  };

  const startPsSession=(deviceId)=>{
    const s={id:`ps_${Date.now()}`,deviceId,deviceName:psDevices.find(d=>d.id===deviceId)?.name||'',startTime:Date.now(),startTimeStr:new Date().toLocaleString('ar-EG'),status:'active',cashierName:currentUser.name};
    const upd=[...psSessions,s];setPsSessions(upd);syncToCloud({psSessions:upd});
  };

  const endPsSession=(sessionId)=>{
    const s=psSessions.find(s=>s.id===sessionId);if(!s)return;
    const dev=psDevices.find(d=>d.id===s.deviceId);if(!dev)return;
    const durMin=Math.ceil((Date.now()-s.startTime)/60000), cost=Math.ceil(durMin/60)*(dev.hourlyRate||0);
    const ended={...s,endTime:Date.now(),endTimeStr:new Date().toLocaleString('ar-EG'),durationMin:durMin,cost,status:'ended'};
    const updS=psSessions.map(ss=>ss.id===sessionId?ended:ss);setPsSessions(updS);
    if(cost>0){
      const psOrd={id:Date.now(),items:[{id:`ps_dev`,name:`${dev.name} - ${durMin} دقيقة`,price:cost,quantity:1}],subtotal:cost,discountAmount:0,tax:0,total:cost,date:new Date().toLocaleString('ar-EG'),timestamp:Date.now(),note:`بلايستيشن - ${dev.name}`,shiftId:activeShift?.id||null,cashierName:currentUser.name};
      const updO=[...orders,psOrd];setOrders(updO);syncToCloud({psSessions:updS,orders:updO});
    } else syncToCloud({psSessions:updS});
  };

  if(!isDataLoaded) return (
    <div className="h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900" dir="rtl">
      <div className="relative mb-6">
        <div className="w-20 h-20 rounded-full border-4 border-indigo-100 flex items-center justify-center"><Coffee className="w-8 h-8 text-indigo-600"/></div>
        <Loader2 className="w-20 h-20 animate-spin text-indigo-600 absolute inset-0"/>
      </div>
      <p className="font-black text-slate-800 text-lg">جاري تحميل المنصة...</p>
    </div>
  );
  ...
  return (
    <ErrorBoundary>
      <div className={isDarkMode?'dark':''}>
        <style>{`
          @media print{body *{visibility:hidden;}.print-section,.print-section *{visibility:visible;}.print-section{position:absolute;left:0;top:0;width:100%;}.no-print{display:none !important;}}
          input[type="number"]::-webkit-outer-spin-button,input[type="number"]::-webkit-inner-spin-button{-webkit-appearance:none;margin:0;}
          input[type="number"]{-moz-appearance:textfield;}
          .custom-scrollbar::-webkit-scrollbar{width:6px;height:6px;}.custom-scrollbar::-webkit-scrollbar-thumb{background-color:#cbd5e1;border-radius:10px;}.dark .custom-scrollbar::-webkit-scrollbar-thumb{background-color:#475569;}
          .no-scrollbar::-webkit-scrollbar{display:none;}.no-scrollbar{-ms-overflow-style:none;scrollbar-width:none;}
          @keyframes pulse-red{0%,100%{opacity:1}50%{opacity:0.5}}.pulse-red{animation:pulse-red 1.5s infinite;}
        `}</style>
        ...
        <div className={`fixed top-0 left-0 right-0 z-[60] text-[10px] md:text-xs font-bold py-1.5 px-3 flex justify-between items-center shadow-md transition-all
          ...
          <div className="flex items-center gap-2">
            {currentUser&&lowStockItems.length>0&&<button onClick={()=>setCurrentRoute('inventory')} className="flex items-center gap-1 bg-white/20 px-2 py-0.5 rounded-full pulse-red"><Bell size={11}/>{lowStockItems.length} مواد منخفضة</button>}
            {currentUser&&(expiredProducts.length>0||nearExpiryProducts.length>0)&&<button onClick={()=>setCurrentRoute('products')} className="flex items-center gap-1 bg-amber-400/30 px-2 py-0.5 rounded-full"><AlertTriangle size={11}/>{expiredProducts.length+nearExpiryProducts.length} صلاحيات</button>}
          </div>
          ...
          <div dir="rtl" className="min-h-screen bg-slate-100 dark:bg-slate-900 flex items-center justify-center font-sans relative overflow-hidden p-4 pt-10">
            ...
            <div className="bg-white dark:bg-slate-800 p-8 md:p-10 rounded-3xl shadow-2xl w-full max-w-md border-t-8 border-indigo-600 z-10 relative">
              <div className="flex justify-between items-start mb-8">
                ...
                <button onClick={()=>setIsDarkMode(!isDarkMode)} className="p-2.5 bg-slate-100 dark:bg-slate-700 rounded-xl text-slate-500 hover:text-indigo-600">
          ...
          <div dir="rtl" className="min-h-screen bg-slate-50 dark:bg-slate-900 w-full overflow-y-auto custom-scrollbar pb-10 pt-7">
            ...
            <div className="p-4 md:p-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 max-w-7xl mx-auto mt-2">
              {(selectedCategoryFilter==='all'?products:products.filter(p=>p.category===selectedCategoryFilter)).map(p=>{
                const op=getProductPriceWithOffer(p), hasOffer=op<p.price;
                return(
                  <div key={p.id} className="bg-white dark:bg-slate-800 rounded-3xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-700 hover:shadow-lg transition-all group flex flex-col relative">
                    ...
                    <div className="p-4 flex flex-col flex-1 justify-between gap-3">
                      ...
                      <div className="flex justify-between items-center border-t border-slate-100 dark:border-slate-700 pt-3">
                        <div>{hasOffer&&<span className="text-slate-400 line-through text-sm ml-2">{p.price} ج</span>}<span className={`font-black text-xl ${hasOffer?'text-rose-500':'text-indigo-600 dark:text-indigo-400'}`}>{op.toFixed(0)} ج.م</span></div>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div dir="rtl" className="flex h-screen bg-slate-50 dark:bg-slate-900 font-sans text-slate-800 dark:text-slate-200 overflow-hidden w-full pt-7">
              ...
              <>
                ...
                <div className={`fixed inset-y-0 right-0 z-50 transform ${isMobileMenuOpen?"translate-x-0":"translate-x-full"} lg:relative lg:translate-x-0 transition-transform duration-300 w-64 md:w-72 bg-white dark:bg-slate-900 flex flex-col shrink-0 pt-7 border-l border-slate-200 dark:border-slate-800 shadow-xl lg:shadow-none`}>
                  <div className="p-4 md:p-6 border-b border-slate-100 dark:border-slate-800 shrink-0 flex justify-between items-center">
                    <div><h2 className="text-lg font-black flex items-center gap-2 text-slate-800 dark:text-white"><Coffee className="text-indigo-500"/> {globalSettings.appName}</h2><p className="text-indigo-600 dark:text-indigo-400 text-xs mt-1 font-bold truncate max-w-[180px]">{currentUser.cafeName}</p></div>
                  ...
                  <nav className="flex-1 p-3 space-y-1 overflow-y-auto custom-scrollbar">
                    {[
                      ...
                      <button key={item.id} onClick={()=>{setCurrentRoute(item.id);setIsMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold transition-all text-sm ${currentRoute===item.id?'bg-indigo-600 text-white shadow-md':'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800 hover:text-indigo-600 dark:hover:text-white'}`}>
                    ...
                    <button onClick={()=>{setCurrentRoute('pos');setIsMobileMenuOpen(false);}} className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold mt-4 border-2 border-indigo-100 dark:border-slate-700 text-sm ${currentRoute==='pos'?'bg-indigo-600 border-indigo-600 text-white':'text-indigo-600 dark:text-slate-300 hover:bg-indigo-50 dark:hover:bg-slate-800'}`}>
                  ...
                  <div className="p-3 border-t border-slate-100 dark:border-slate-800 shrink-0">
                    <div className="flex items-center justify-between mb-2 px-1">
                      ...
                      <button onClick={()=>{const v=!isTaxEnabled;setIsTaxEnabled(v);syncToCloud({isTaxEnabled:v});}} className={`w-12 h-6 rounded-full transition-colors relative ${isTaxEnabled?'bg-indigo-600':'bg-slate-300 dark:bg-slate-600'}`}>
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative w-full">
              <header className="p-3 md:p-4 md:px-8 flex justify-between items-center shadow-sm z-30 shrink-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  <button onClick={()=>setIsDarkMode(!isDarkMode)} className="p-2 rounded-lg bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-white">{isDarkMode?<Sun size={17}/>:<Moon size={17}/>}</button>
                ...
                {currentUser.role==='cashier'&&!activeShift&&activeModal!=='closeShift'?(
                  <div className="absolute inset-0 z-30 flex items-center justify-center bg-slate-100 dark:bg-slate-900 p-4">
                    <div className="bg-white dark:bg-slate-800 p-6 md:p-10 rounded-3xl shadow-2xl max-w-md w-full text-center border border-slate-200 dark:border-slate-700">
                      ...
                      <form onSubmit={(e)=>{e.preventDefault();const ns={id:'sh_'+Date.now(),cashierName:currentUser.name,startTime:new Date().toLocaleString('ar-EG'),timestamp:Date.now(),startingCash:parseFloat(e.target.startingCash.value)||0,status:'open'};const u=[...shifts,ns];setShifts(u);syncToCloud({shifts:u});}}>
                        <div className="text-right mb-5"><label className="block text-sm font-black mb-2 dark:text-white">المبلغ الفعلي في الدرج</label><input required name="startingCash" type="number" min="0" step="any" placeholder="0.00" className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-700 rounded-2xl text-center font-black text-2xl focus:outline-none focus:border-indigo-500 text-slate-800 dark:text-white"/></div>
                            ...
                            {tenants.map(cafe=>(
                              <tr key={cafe.id} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 text-sm">
                                ...
                                <td className="p-5 text-center flex justify-center gap-2">
                                  <button onClick={()=>{const u=tenants.map(t=>t.id===cafe.id?{...t,status:t.status==='active'?'suspended':'active'}:t);setTenants(u);syncPlatformToCloud({tenants:u});}} className="bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 px-4 py-1.5 rounded-xl text-xs font-bold text-slate-800 dark:text-white">تبديل</button>
                                  <button onClick={()=>{setFormData(cafe);setActiveModal('tenant');}} className="bg-indigo-100 text-indigo-700 hover:bg-indigo-200 px-4 py-1.5 rounded-xl text-xs font-bold">تعديل</button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                  ...
                  <div className="flex flex-col lg:flex-row h-full p-2 md:p-4 lg:p-6 gap-4 lg:gap-6 overflow-hidden relative">
                    <div className="flex-1 flex flex-col gap-4 overflow-hidden pb-16 lg:pb-0">
                      <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-700 w-fit">
                        <button onClick={()=>{setOrderType('takeaway');setActiveTableId(null);}} className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${orderType==='takeaway'?'bg-indigo-600 text-white shadow-md':'text-slate-600 dark:text-slate-400'}`}>تيك أواي</button>
                        <button onClick={()=>setOrderType('dine_in')} className={`px-6 py-2.5 rounded-xl font-bold text-sm transition-all ${orderType==='dine_in'?'bg-indigo-600 text-white shadow-md':'text-slate-600 dark:text-slate-400'}`}>صالة</button>
                      </div>

                      {orderType==='dine_in'&&!activeTableId&&(
                        <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-3">
                          {tables.map(t=>{const ti=activeTableOrders[t.id],isOcc=Array.isArray(ti)&&ti.length>0;return(
                            <button key={t.id} onClick={()=>{setActiveTableId(t.id);setCart(Array.isArray(activeTableOrders[t.id])?activeTableOrders[t.id]:[]); }} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 transition-all ${isOcc?'border-amber-400 bg-amber-50 dark:bg-amber-900/20 text-amber-700':'border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-800 hover:border-indigo-400 text-slate-700 dark:text-slate-300'}`}>
                              <Armchair className="w-7 h-7"/><span className="font-black text-xs line-clamp-1">{t.name}</span>
                              {isOcc&&<span className="text-[9px] bg-amber-500 text-white px-2 py-0.5 rounded-full font-bold">{ti.length} صنف</span>}
                            </button>
                          );})}
                        </div>
                      )}
                      ...
                      <div className="flex gap-2 overflow-x-auto no-scrollbar py-1">
                        <button onClick={()=>setSelectedCategoryFilter('all')} className={`whitespace-nowrap px-5 py-2 rounded-xl font-bold text-sm transition-all ${selectedCategoryFilter==='all'?'bg-indigo-600 text-white':'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>الكل</button>
                        {categories.map(cat=><button key={cat.id} onClick={()=>setSelectedCategoryFilter(cat.id)} className={`whitespace-nowrap px-5 py-2 rounded-xl font-bold text-sm transition-all ${selectedCategoryFilter===cat.id?'bg-indigo-600 text-white':'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>{cat.name}</button>)}
                      </div>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 overflow-y-auto pr-1 custom-scrollbar">
                        {(selectedCategoryFilter==='all'?products:products.filter(p=>p.category===selectedCategoryFilter)).map(p=>{
                          const op=getProductPriceWithOffer(p), hasOffer=op<p.price;
                          return(
                            <button key={p.id} onClick={()=>{if(orderType==='dine_in'&&!activeTableId)return alert('الرجاء اختيار طاولة.');const pr=getProductPriceWithOffer(p);const ex=cart.find(i=>i.id===p.id);if(ex)setCart(cart.map(i=>i.id===p.id?{...i,quantity:i.quantity+1}:i));else setCart([...cart,{...p,price:pr,originalPrice:p.price,quantity:1}]);}} className="bg-white dark:bg-slate-800 p-4 rounded-3xl shadow-sm border border-slate-200 dark:border-slate-700 hover:border-indigo-500 hover:shadow-md transition-all flex flex-col items-center text-center gap-2 group relative">
                    ...
                    <div className="lg:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 p-3 border-t border-slate-200 dark:border-slate-700 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-30 flex justify-between items-center">
                      <div className="font-black text-base text-indigo-600 dark:text-indigo-400">
                        {(()=>{const s=cart.reduce((x,i)=>x+(i.price*i.quantity),0);const dv=parseFloat(discountValue)||0;const d=currentUser.role==='admin'&&dv>0?(discountType==='percent'?Math.min(s,s*dv/100):Math.min(s,dv)):0;return((s-d)*(isTaxEnabled?1.14:1)).toFixed(2);})())} ج
                      </div>
                      <button onClick={()=>setIsMobileCartOpen(true)} className="bg-indigo-600 text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 text-sm shadow-lg"><ShoppingCart size={17}/> السلة ({cart.length})</button>
                    </div>
                    ...
                    <div className={`w-full lg:w-[350px] xl:w-[420px] bg-white dark:bg-slate-800 rounded-t-3xl lg:rounded-3xl shadow-2xl lg:shadow-md border border-slate-200 dark:border-slate-700 flex flex-col h-[85vh] lg:h-full fixed lg:relative bottom-0 left-0 right-0 z-40 transition-transform duration-300 ${isMobileCartOpen?'translate-y-0':'translate-y-full lg:translate-y-0'}`}>
                      <div className="p-5 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 rounded-t-3xl flex justify-between items-center shrink-0">
                        ...
                        <div className="flex gap-2">
                          {activeTableId&&<button onClick={()=>{setCart([]);setActiveTableId(null);setIsMobileCartOpen(false);}} className="text-[10px] font-bold text-rose-500 bg-rose-50 px-2 py-1 rounded-lg">إلغاء</button>}
                      ...
                      <div className="flex-1 overflow-auto p-4 space-y-2 custom-scrollbar">
                        {cart.length===0?<div className="text-center text-slate-400 mt-20"><Package className="w-12 h-12 mx-auto mb-3 opacity-20"/><p className="font-bold text-sm">السلة فارغة</p></div>:cart.map(item=>(
                      ...
                      <div className="p-5 bg-slate-50 dark:bg-slate-800/80 border-t border-slate-100 dark:border-slate-700 lg:rounded-b-3xl shrink-0">
                          ...
                          <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-2xl border border-amber-200 dark:border-amber-800">
                            ...
                            <div className="flex gap-2">
                              <div className="flex bg-white dark:bg-slate-800 rounded-xl border border-amber-200 dark:border-amber-800 p-0.5 shrink-0">
                                <button type="button" onClick={()=>setDiscountType('percent')} className={`px-2.5 py-1.5 rounded-lg text-xs font-black transition-all ${discountType==='percent'?'bg-amber-500 text-white':'text-amber-600 dark:text-amber-400'}`}>%</button>
                                <button type="button" onClick={()=>setDiscountType('fixed')} className={`px-2.5 py-1.5 rounded-lg text-xs font-black transition-all ${discountType==='fixed'?'bg-amber-500 text-white':'text-amber-600 dark:text-amber-400'}`}>ج</button>
                              </div>
                              <input type="number" min="0" step="any" value={discountValue} onChange={e=>setDiscountValue(e.target.value)} placeholder={discountType==='percent'?'نسبة %':'مبلغ'} className="flex-1 p-2 bg-white dark:bg-slate-800 border border-amber-200 dark:border-amber-800 rounded-xl text-xs font-bold text-amber-700 dark:text-amber-300 outline-none focus:border-amber-500 text-center"/>
                        ...
                        {(()=>{const s=cart.reduce((x,i)=>x+(i.price*i.quantity),0);const dv=parseFloat(discountValue)||0;const d=currentUser.role==='admin'&&dv>0?(discountType==='percent'?Math.min(s,s*dv/100):Math.min(s,dv)):0;const af=s-d;const t=isTaxEnabled?af*0.14:0;const tot=af+t;return(
                          <div className="space-y-1.5 mb-4">
                            <div className="flex justify-between text-sm font-bold text-slate-500"><span>المجموع:</span><span>{s.toFixed(2)} ج</span></div>
                            {d>0&&<div className="flex justify-between text-sm font-black text-emerald-600"><span>الخصم:</span><span>- {d.toFixed(2)} ج</span></div>}
                            {isTaxEnabled&&<div className="flex justify-between text-sm font-bold text-slate-500"><span>ضريبة 14%:</span><span>{t.toFixed(2)} ج</span></div>}
                            <div className="flex justify-between items-center font-black text-2xl pt-2 border-t border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white"><span>الإجمالي:</span><span className="text-indigo-600 dark:text-indigo-400">{tot.toFixed(2)} ج</span></div>
                          </div>
                        );})()}
                        {orderType==='dine_in'&&activeTableId?(
                          <div className="flex gap-3">
                            <button onClick={async()=>{if(isHoldingTable||cart.length===0)return;setIsHoldingTable(true);try{const upd={...activeTableOrders,[activeTableId]:[...cart]};setActiveTableOrders(upd);await syncToCloud({activeTableOrders:upd});setCart([]);setActiveTableId(null);setOrderType('takeaway');setIsMobileCartOpen(false);}finally{setIsHoldingTable(false);}}} disabled={cart.length===0||isHoldingTable} className={`flex-1 py-4 rounded-2xl font-bold flex items-center justify-center gap-2 disabled:opacity-50 text-white text-sm ${isHoldingTable?'bg-amber-400 cursor-wait':'bg-amber-500 hover:bg-amber-600'}`}>
                    ...
                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                      ...
                      <div className="flex gap-1.5 p-1.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl overflow-x-auto no-scrollbar w-fit">
                        {['daily','weekly','monthly','quarterly','semi','yearly','all'].map(p=>(
                          <button key={p} onClick={()=>setReportPeriod(p)} className={`px-4 py-2 rounded-lg font-bold text-xs whitespace-nowrap transition-colors ${reportPeriod===p?'bg-indigo-600 text-white':'text-slate-500 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-700'}`}>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
                      ...
                      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 mb-6">
                        {[{v:'daily',l:'يومي'},{v:'weekly',l:'أسبوعي'},{v:'monthly',l:'شهري'},{v:'quarterly',l:'ربع سنوي'},{v:'semi',l:'نصف سنوي'},{v:'yearly',l:'سنوي'}].map(p=>(
                          <button key={p.v} onClick={()=>setReportPeriod(p.v)} className={`py-3 rounded-xl font-bold text-sm transition-all ${reportPeriod===p.v?'bg-indigo-600 text-white shadow-md':'bg-slate-50 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-indigo-50'}`}>{p.l}</button>
                        ))}
                      </div>
                          ...
                          <tbody>
                            {rawMaterials.map(rm=>(
                              <tr key={rm.id} className={`border-b border-slate-100 dark:border-slate-700 text-sm ${rm.currentStock<=stockAlertThreshold?'bg-rose-50/50 dark:bg-rose-900/10':''}`}>
                                <td className="p-5 font-black text-slate-800 dark:text-white flex items-center gap-2">{rm.currentStock<=stockAlertThreshold&&<Bell size={14} className="text-rose-500 shrink-0"/>}{rm.name}</td>
                          ...
                          return(
                            <div key={offer.id} className={`bg-white dark:bg-slate-800 p-5 rounded-3xl border-2 shadow-sm ${isAct?'border-emerald-300 dark:border-emerald-700':'border-slate-200 dark:border-slate-700 opacity-60'}`}>
                              ...
                              <div className="flex gap-2 mt-4">
                                <button onClick={()=>{const u=offers.map(o=>o.id===offer.id?{...o,isActive:!o.isActive}:o);setOffers(u);syncToCloud({offers:u});}} className={`flex-1 py-2 rounded-xl text-xs font-black transition-colors ${offer.isActive?'bg-slate-100 text-slate-600 hover:bg-slate-200':'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'}`}>{offer.isActive?'إيقاف':'تفعيل'}</button>
                                <button onClick={()=>{setDeleteConfig({type:'offer',id:offer.id});setActiveModal('delete');}} className="bg-rose-50 text-rose-500 hover:bg-rose-100 p-2 rounded-xl"><Trash2 size={14}/></button>
                              </div>
                            </div>
                          );
                        })}
                          ...
                          <tbody>
                            {employees.map(emp=>(
                              <tr key={emp.id} className="border-b border-slate-100 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50 text-sm">
                                ...
                                <td className="p-5 text-center"><button onClick={()=>setSelectedEmployeeReport(emp)} className="bg-indigo-100 text-indigo-700 hover:bg-indigo-200 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1 mx-auto"><User size={12}/> تفاصيل</button></td>
            ...
            {activeModal==='closeShift'&&activeShift&&(
              <CustomModal title="تقفيل الوردية" onClose={closeModal}>
                <form onSubmit={(e)=>{e.preventDefault();const ac=parseFloat(e.target.actualCash.value)||0;const ss=orders.filter(o=>o.shiftId===activeShift.id).reduce((s,o)=>s+o.total,0);const u=shifts.map(s=>s.id===activeShift.id?{...s,endTime:new Date().toLocaleString('ar-EG'),actualCash:ac,totalSales:ss,status:'closed'}:s);setShifts(u);syncToCloud({shifts:u});closeModal();handleLogout();}}>
                    ...
                    <div className="space-y-2 max-h-40 overflow-auto custom-scrollbar">
                      {(formData.recipe||[]).map((item,idx)=>(
                        <div key={idx} className="flex gap-2">
                          ...
                          <button type="button" onClick={()=>{const r=[...formData.recipe];r.splice(idx,1);setFormData({...formData,recipe:r});}} className="text-rose-500 bg-rose-50 dark:bg-rose-900/30 hover:bg-rose-100 p-2.5 rounded-xl"><Trash2 size={15}/></button>
                        </div>
                      ))}
                    </div>
                  </div>

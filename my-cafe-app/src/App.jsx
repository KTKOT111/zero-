...
import {
  ...
  Undo, History, Gamepad, Gift, Bell, FileBarChart, Clock, Download, AlertTriangle
} from 'lucide-react';
  ...
  render() {
    if (this.state.hasError) return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900 p-6 text-center" dir="rtl">
        ...
        <pre className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow border border-rose-100 dark:border-rose-900 text-left text-xs overflow-auto max-w-2xl w-full text-rose-600 dark:text-rose-400" dir="ltr">{String(this.state.error)}</pre>
...
try {
  ...
  secondaryApp = initializeApp(firebaseConfig, "SecondaryApp"); secondaryAuth = getAuth(secondaryApp);
} catch (e) { console.error(e); }

const defaultProducts = [{ id: 1, name: 'اسبريسو', category: 'مشروبات', price: 35, stock: 500, image: '' }];
...
export default function App() {
  ...
  const fbUserRef = useRef(null); const currentUserRef = useRef(null);
  ...
  useEffect(() => { setIsOnline(navigator.onLine); const on = () => setIsOnline(true); const off = () => setIsOnline(false); window.addEventListener('online', on); window.addEventListener('offline', off); return () => { window.removeEventListener('online', on); window.removeEventListener('offline', off); }; }, []);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'coffee_erp_platform', 'config'), (snap) => {
      if (snap.exists()) { const data = snap.data(); if (data.globalSettings) setGlobalSettings(data.globalSettings); if (data.tenants) setTenants(data.tenants); }
    });
    return () => unsub();
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(doc(db, 'coffee_erp_cafes', currentUser.cafeId), (snap) => {
      if (snap.exists()) {
  ...
  const syncPlatformToCloud = useCallback(async (newData) => { if (!db || !fbUserRef.current) return; try { await setDoc(doc(db, 'coffee_erp_platform', 'config'), { ...newData, lastUpdated: new Date().toISOString() }, { merge: true }); } catch(err) {} }, []);
  const syncToCloud = useCallback(async (newData) => {
    const cafeId = currentUserRef.current?.cafeId; if (!db || !fbUserRef.current || !cafeId) return;
    setSyncStatus('saving');
    try { await setDoc(doc(db, 'coffee_erp_cafes', cafeId), { ...newData, lastUpdated: new Date().toISOString() }, { merge: true }); setSyncStatus('success'); setTimeout(() => setSyncStatus('idle'), 2000); } catch(err) { setSyncStatus('error'); }
  }, []);

  const handleLogin = async (e) => { e.preventDefault(); setLoginError(''); setIsLoggingIn(true); try { await signInWithEmailAndPassword(auth, email, password); } catch (e) { setLoginError('بيانات غير صحيحة.'); } finally { setIsLoggingIn(false); } };
  ...
  const activeShift = useMemo(() => { if (!currentUser) return null; return shifts.find(s => s.status === 'open' && s.cashierName === currentUser.name); }, [shifts, currentUser]);
  ...
  const processVoidOrder = () => {
    const orderToVoid = formData.order; if (!orderToVoid || orderToVoid.status === 'voided') return;
    const newRawMaterials = [...rawMaterials];
    orderToVoid.items.forEach(cartItem => {
      const product = products.find(p => p.id === cartItem.id);
      if (product?.recipe) { product.recipe.forEach(ing => { const idx = newRawMaterials.findIndex(rm => rm.id === ing.materialId); if (idx !== -1) newRawMaterials[idx].currentStock += (ing.amount * cartItem.quantity); }); }
    });
    ...
    setRawMaterials(newRawMaterials); setOrders(updatedOrders); syncToCloud({ rawMaterials: newRawMaterials, orders: updatedOrders }); closeModal();
  };
  ...
  const financialMetrics = useMemo(() => {
    ...
    let totalCogs = 0;
    fOrders.forEach(order => { order.items.forEach(item => { const product = products.find(p => p.id === item.id); if (product?.recipe) { product.recipe.forEach(ingredient => { const rawMat = rawMaterials.find(rm => rm.id === ingredient.materialId); if (rawMat) totalCogs += (ingredient.amount * item.quantity * rawMat.costPerUnit); }); } }); });
    return { totalRevenue, totalExpenses: totalExp, totalCogs, netProfit: totalRevenue - (totalExp + totalCogs), filteredOrders: fOrders, filteredExpenses: fExpenses };
  ...
  const confirmDelete = () => {
    let updates = {};
    if (deleteConfig.type === 'tenant') { const u = tenants.filter(t => t.id !== deleteConfig.id); setTenants(u); syncPlatformToCloud({ tenants: u }); }
    else {
  ...
  const genericSave = (collectionName, stateArray, setterFunc, extraFormat = {}) => {
    const updated = formData.id ? stateArray.map(item => item.id === formData.id ? { ...item, ...formData, ...extraFormat } : item) : [...stateArray, { ...formData, ...extraFormat, id: `${collectionName}_${Date.now()}` }];
  ...
  if (!isDataLoaded) return <div className="h-screen flex flex-col items-center justify-center bg-slate-50 dark:bg-slate-900"><Loader2 className="w-16 h-16 animate-spin text-indigo-600 mb-4" /><p className="font-black dark:text-slate-300">جاري التحميل...</p></div>;
    ...
    <ErrorBoundary>
      <div className={isDarkMode ? 'dark' : ''}>
        ...
        <div className={`fixed top-0 left-0 right-0 z-[60] text-[10px] md:text-xs font-bold py-1.5 px-3 flex justify-between items-center shadow-md transition-all duration-300 ${!isOnline ? 'bg-rose-600' : syncStatus === 'error' ? 'bg-rose-600' : syncStatus === 'saving' || isSyncing ? 'bg-amber-500' : 'bg-emerald-600'} text-white`}>
          <div className="flex items-center gap-1.5">{!isOnline ? <WifiOff size={13}/> : syncStatus === 'saving' || isSyncing ? <RefreshCw size={13} className="animate-spin"/> : syncStatus === 'error' ? <AlertCircle size={13}/> : <Wifi size={13}/>}<span>{!isOnline ? 'أوفلاين' : syncStatus === 'error' ? `خطأ: ${syncError}` : syncStatus === 'saving' ? 'جاري المزامنة...' : 'متصل'}</span></div>
        </div>
        ...
        {!currentUser ? (
          <div dir="rtl" className="min-h-screen bg-slate-100 dark:bg-slate-900 flex items-center justify-center p-4 pt-10">
            <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-2xl w-full max-w-md border-t-8 border-indigo-600">
              <div className="flex justify-between mb-8"><div className="flex items-center gap-4"><div className="bg-indigo-50 dark:bg-indigo-900/50 p-4 rounded-2xl text-indigo-600"><Coffee size={36}/></div><div><h1 className="text-3xl font-black dark:text-white">{globalSettings.appName}</h1></div></div><button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2.5 bg-slate-100 dark:bg-slate-700 rounded-xl dark:text-white">{isDarkMode ? <Sun size={20}/> : <Moon size={20}/>}</button></div>
            ...
            {currentUser.role === 'admin' && (
              <>
                ...
                <div className={`fixed inset-y-0 right-0 z-50 transform ${isMobileMenuOpen ? "translate-x-0" : "translate-x-full"} lg:relative lg:translate-x-0 transition-transform duration-300 w-64 bg-white dark:bg-slate-900 flex flex-col pt-7 border-l border-slate-200 dark:border-slate-800`}>
                  <div className="p-5 border-b border-slate-100 dark:border-slate-800 shrink-0 flex justify-between items-center"><div><h2 className="text-lg font-black dark:text-white flex items-center gap-2"><Coffee className="text-indigo-500"/> {globalSettings.appName}</h2></div><button onClick={() => setIsMobileMenuOpen(false)} className="lg:hidden p-2 bg-slate-100 dark:bg-slate-800 rounded-lg"><X size={18} /></button></div>
                  <nav className="flex-1 p-3 space-y-1.5 overflow-y-auto custom-scrollbar">
                    {[{ id: 'dashboard', icon: <LayoutDashboard size={19}/>, label: 'لوحة القيادة' }, { id: 'reports', icon: <FileBarChart size={19}/>, label: 'التقارير المجمعة' }, { id: 'invoices', icon: <FileText size={19}/>, label: 'سجل الفواتير' }, { id: 'shifts', icon: <ClipboardList size={19}/>, label: 'سجل الورديات' }, { id: 'inventory', icon: <Package size={19}/>, label: 'المواد الخام (المخزن)' }, { id: 'stock_take', icon: <History size={19}/>, label: 'جرد وتسوية' }, { id: 'products', icon: <Coffee size={19}/>, label: 'المنتجات' }, { id: 'tables', icon: <Utensils size={19}/>, label: 'إدارة الصالة' }, { id: 'playstation', icon: <Gamepad size={19}/>, label: 'البلايستيشن' }, { id: 'offers', icon: <Gift size={19}/>, label: 'إدارة العروض' }, { id: 'hr', icon: <Users size={19}/>, label: 'شؤون الموظفين' }, { id: 'expenses', icon: <Receipt size={19}/>, label: 'المصروفات' }].map(item => (<button key={item.id} onClick={() => { setCurrentRoute(item.id); setIsMobileMenuOpen(false); }} className={`w-full flex items-center gap-3 p-3 rounded-xl font-bold text-sm ${currentRoute === item.id ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>{item.icon} {item.label}</button>))}
                    <button onClick={() => { setCurrentRoute('pos'); setIsMobileMenuOpen(false); }} className="w-full flex items-center gap-3 p-3 rounded-xl font-bold mt-4 border-2 border-indigo-100 dark:border-slate-700 text-indigo-600 dark:text-slate-300"><ShoppingCart size={19}/> نقطة البيع</button>
                  </nav>
                </div>
              </>
            )}
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden relative">
              <header className="p-3 md:p-4 flex justify-between items-center shadow-sm bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 print:hidden">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                  ...
                  {currentUser.role === 'cashier' && activeShift && <button onClick={() => setActiveModal('closeShift')} className="bg-rose-50 text-rose-600 px-3 py-1.5 rounded-lg text-xs font-bold flex gap-1"><Power size={14}/><span className="hidden md:inline">إنهاء الوردية</span></button>}
                  <button onClick={() => setIsDarkMode(!isDarkMode)} className="p-2 rounded-lg bg-slate-100 dark:bg-slate-700 dark:text-white">{isDarkMode ? <Sun size={17} /> : <Moon size={17} />}</button>
              ...
              <div className="flex-1 overflow-auto custom-scrollbar relative bg-slate-50 dark:bg-slate-900">
                {currentUser.role === 'cashier' && !activeShift && activeModal !== 'closeShift' ? (
                  <div className="absolute inset-0 z-30 flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-900"><div className="bg-white dark:bg-slate-800 p-8 rounded-3xl shadow-2xl w-full max-w-md text-center"><Play className="w-16 h-16 text-indigo-600 mx-auto mb-4"/><h2 className="text-2xl font-black dark:text-white mb-6">استلام العهدة</h2><form onSubmit={e => { e.preventDefault(); setShifts([...shifts, { id: 'sh_'+Date.now(), cashierName: currentUser.name, startTime: new Date().toLocaleString('ar-EG'), timestamp: Date.now(), startingCash: parseFloat(e.target.startingCash.value)||0, status: 'open' }]); syncToCloud({ shifts }); }}><input required name="startingCash" type="number" min="0" step="any" placeholder="المبلغ (ج)" className="w-full p-4 mb-4 bg-slate-50 dark:bg-slate-900 border-2 rounded-xl text-center font-black text-2xl dark:text-white outline-none"/><button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-xl font-black">بدء الوردية</button></form></div></div>
                ) :
                ...
                currentRoute === 'pos' || currentUser.role === 'cashier' ? (
                  <div className="flex flex-col lg:flex-row h-full p-2 md:p-4 gap-4 relative">
                    <div className="flex-1 flex flex-col gap-4 overflow-hidden">
                      ...
                      {orderType === 'dine_in' && !activeTableId && (
                        <div className="grid grid-cols-3 sm:grid-cols-4 gap-3"><div className="col-span-full font-bold dark:text-white mb-2">الطاولات:</div>{tables.map(t => { const isOcc = activeTableOrders[t.id]?.length > 0; return (<button key={t.id} onClick={() => { setActiveTableId(t.id); setCart(activeTableOrders[t.id] || []); }} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-2 ${isOcc ? 'border-amber-400 bg-amber-50 dark:bg-amber-900/30' : 'border-slate-200 bg-white dark:bg-slate-800 dark:border-slate-700'}`}><Armchair className={isOcc ? 'text-amber-600' : 'text-slate-400'}/><span className="font-black text-xs dark:text-white">{t.name}</span>{isOcc && <span className="text-[9px] bg-amber-500 text-white px-2 py-0.5 rounded-full">{activeTableOrders[t.id].length} صنف</span>}</button>); })}</div>
                      )}
                      <div className="flex gap-2 overflow-x-auto no-scrollbar py-1"><button onClick={() => setSelectedCategoryFilter('all')} className={`px-5 py-2 rounded-xl font-bold text-sm whitespace-nowrap ${selectedCategoryFilter === 'all' ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>الكل</button>{categories.map(c => <button key={c.id} onClick={() => setSelectedCategoryFilter(c.id)} className={`px-5 py-2 rounded-xl font-bold text-sm whitespace-nowrap ${selectedCategoryFilter === c.id ? 'bg-indigo-600 text-white' : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300'}`}>{c.name}</button>)}</div>
                      <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 overflow-y-auto custom-scrollbar">{(selectedCategoryFilter === 'all' ? products : products.filter(p => p.category === selectedCategoryFilter)).map(p => (<button key={p.id} onClick={() => { if (orderType === 'dine_in' && !activeTableId) return alert('حدد الطاولة!'); const ex = cart.find(i => i.id === p.id); if (ex) setCart(cart.map(i => i.id === p.id ? { ...i, quantity: i.quantity + 1 } : i)); else setCart([...cart, { ...p, quantity: 1 }]); }} className="bg-white dark:bg-slate-800 p-3 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col items-center text-center gap-2"><div className="w-14 h-14 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-500 rounded-full flex items-center justify-center"><Coffee size={20}/></div><h3 className="font-bold text-xs dark:text-white">{p.name}</h3><p className="text-indigo-600 dark:text-indigo-400 font-black text-sm">{p.price} ج</p></button>))}</div>
                    </div>
                    <div className="w-full lg:w-[350px] bg-white dark:bg-slate-800 rounded-3xl shadow-xl flex flex-col h-full border dark:border-slate-700">
                      <div className="p-4 border-b dark:border-slate-700 flex justify-between items-center"><h3 className="font-black flex items-center gap-2 dark:text-white"><ShoppingCart className="w-5 h-5 text-indigo-500"/> السلة</h3>{activeTableId && <button onClick={() => { setCart([]); setActiveTableId(null); }} className="text-xs font-bold text-rose-500 bg-rose-50 dark:bg-rose-900/30 px-2 py-1 rounded-lg">إلغاء</button>}</div>
                      <div className="flex-1 overflow-auto p-4 space-y-2 custom-scrollbar bg-slate-50 dark:bg-slate-900/30">{cart.map(item => (<div key={item.id} className="bg-white dark:bg-slate-800 p-2.5 rounded-2xl border dark:border-slate-700 flex justify-between items-center"><div><p className="font-bold text-xs dark:text-white">{item.name}</p><p className="text-[10px] font-black text-indigo-600">{item.price * item.quantity} ج</p></div><div className="flex items-center gap-1"><button onClick={() => setCart(cart.map(i => i.id === item.id ? { ...i, quantity: i.quantity + 1 } : i))} className="text-emerald-500 bg-slate-50 dark:bg-slate-900 p-1 rounded-lg"><Plus size={14}/></button><span className="font-black text-xs w-5 text-center dark:text-white">{item.quantity}</span><button onClick={() => { if (item.quantity > 1) setCart(cart.map(i => i.id === item.id ? { ...i, quantity: i.quantity - 1 } : i)); else setCart(cart.filter(i => i.id !== item.id)); }} className="text-rose-500 bg-slate-50 dark:bg-slate-900 p-1 rounded-lg"><Minus size={14}/></button></div></div>))}</div>
                      <div className="p-4 border-t dark:border-slate-700">
                        {offers.filter(o => o.active).length > 0 && (
                           <select value={selectedOfferId} onChange={e => setSelectedOfferId(e.target.value)} className="w-full mb-3 p-2 border rounded-xl text-xs font-bold bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 outline-none"><option value="">بدون عروض</option>{offers.filter(o => o.active).map(o => <option key={o.id} value={o.id}>{o.name} ({o.discountType === 'percent' ? o.discountValue+'%' : o.discountValue+'ج'})</option>)}</select>
                        )}
                        {(() => {
                          const sub = cart.reduce((s, i) => s + (i.price * i.quantity), 0);
                          let disc = 0; if (selectedOfferId) { const off = offers.find(o => o.id === selectedOfferId); if (off) disc = off.discountType === 'percent' ? sub * off.discountValue / 100 : off.discountValue; }
                          disc = Math.min(sub, disc); const tot = sub - disc + (isTaxEnabled ? (sub - disc) * taxRate : 0);
                          return (<div className="mb-4 space-y-1"><div className="flex justify-between text-sm font-bold text-slate-500"><span>المجموع:</span><span>{sub.toFixed(2)}</span></div>{disc > 0 && <div className="flex justify-between text-sm font-black text-emerald-600"><span>الخصم:</span><span>-{disc.toFixed(2)}</span></div>}<div className="flex justify-between font-black text-xl border-t dark:border-slate-700 pt-2 dark:text-white"><span>الإجمالي:</span><span className="text-indigo-600">{tot.toFixed(2)} ج</span></div></div>);
                        })()}
                        {orderType === 'dine_in' && activeTableId ? (<div className="flex gap-2"><button onClick={() => { setActiveTableOrders({...activeTableOrders, [activeTableId]: cart}); syncToCloud({activeTableOrders: {...activeTableOrders, [activeTableId]: cart}}); setCart([]); setActiveTableId(null); setOrderType('takeaway'); }} disabled={cart.length===0} className="flex-1 bg-amber-500 text-white py-3 rounded-xl font-bold text-sm">تعليق</button><button onClick={processOrder} disabled={cart.length===0} className="flex-1 bg-emerald-600 text-white py-3 rounded-xl font-bold text-sm">تقفيل</button></div>) : (<button onClick={processOrder} disabled={cart.length===0} className="w-full bg-indigo-600 text-white py-4 rounded-xl font-black text-lg">دفع</button>)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-4 md:p-8 max-w-7xl mx-auto">
                    <div className="flex flex-col sm:flex-row justify-between items-center mb-8 gap-4 print:hidden">
                      <div className="flex items-center gap-3"><FileBarChart className="text-indigo-600 w-8 h-8"/><h2 className="text-3xl font-black dark:text-white">التقارير الشاملة</h2></div>
                    ...
                    <div className="mb-6 flex gap-2 overflow-x-auto print:hidden">
                      {[{id:'daily',n:'اليوم'},{id:'weekly',n:'أسبوع'},{id:'monthly',n:'شهر'},{id:'quarterly',n:'ربع سنوي'},{id:'semi',n:'نصف سنوي'},{id:'yearly',n:'سنة'},{id:'all',n:'الكل'}].map(p => <button key={p.id} onClick={() => setReportPeriod(p.id)} className={`px-4 py-2 rounded-xl font-bold text-sm whitespace-nowrap ${reportPeriod === p.id ? 'bg-slate-800 text-white' : 'bg-white text-slate-600 border'}`}>{p.n}</button>)}
                    </div>
                    <div className="print-section bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
                      <div className="hidden print:block text-center mb-8 border-b pb-4"><h1 className="text-3xl font-black">{globalSettings.appName}</h1><p className="text-lg font-bold text-slate-500 mt-2">تقرير مالي ({reportPeriod})</p></div>
                      ...
                      <h3 className="font-black text-lg mb-4">تفاصيل المعاملات:</h3>
                      <table className="w-full text-right text-sm border-collapse">
                        <thead><tr className="bg-slate-100 border-b border-slate-300"><th className="p-3">النوع</th><th className="p-3">التاريخ</th><th className="p-3">البيان/رقم الفاتورة</th><th className="p-3">المبلغ</th></tr></thead>
                        <tbody>
                  ...
                  <div className="p-4 md:p-8 max-w-7xl mx-auto">
                    <div className="flex justify-between items-center mb-8"><div className="flex items-center gap-3"><Gamepad className="text-indigo-600 w-8 h-8"/><h2 className="text-3xl font-black dark:text-white">أجهزة البلايستيشن</h2></div><button onClick={() => openModal('psDevice')} className="bg-indigo-600 text-white px-4 py-2.5 rounded-xl font-bold flex gap-2"><Plus size={17}/> جهاز جديد</button></div>
                      ...
                      {psDevices.map(d => {
                        ...
                        return (
                          <div key={d.id} className={`p-5 rounded-3xl border-2 relative overflow-hidden ${isPlaying ? 'bg-amber-50 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800' : 'bg-white border-slate-200 dark:bg-slate-800 dark:border-slate-700'}`}>
                            ...
                            {isPlaying ? (
                              <div className="space-y-3"><div className="flex justify-between font-bold text-sm"><span className="text-amber-700">الوقت:</span><span className="text-amber-700 flex gap-1 items-center"><Clock size={14}/> {Math.floor(elapsedMins/60)}س {elapsedMins%60}د</span></div><div className="flex justify-between font-black text-lg"><span className="text-amber-800">الحساب:</span><span className="text-amber-800">{currentCost} ج</span></div><button onClick={() => handlePsAction(d)} className="w-full py-3 bg-amber-600 text-white rounded-xl font-black shadow-lg">إنهاء ودفع</button></div>
                            ) : (
                  ...
                  <div className="p-4 md:p-8 max-w-7xl mx-auto">
                    <div className="flex justify-between items-center mb-8"><div className="flex items-center gap-3"><Gift className="text-indigo-600 w-8 h-8"/><h2 className="text-3xl font-black dark:text-white">إدارة العروض والخصومات</h2></div><button onClick={() => openModal('offer')} className="bg-indigo-600 text-white px-4 py-2.5 rounded-xl font-bold flex gap-2"><Plus size={17}/> عرض جديد</button></div>
                    <div className="bg-white dark:bg-slate-800 rounded-3xl border overflow-hidden"><table className="w-full text-right"><thead className="bg-slate-50 dark:bg-slate-800/80 border-b text-sm font-bold"><tr><th className="p-5">اسم العرض</th><th className="p-5">القيمة</th><th className="p-5 text-center">الحالة</th><th className="p-5 text-center">حذف</th></tr></thead><tbody>
                      {offers.map(o => (<tr key={o.id} className="border-b text-sm"><td className="p-5 font-black dark:text-white">{o.name}</td><td className="p-5 font-bold text-indigo-600">{o.discountType === 'percent' ? `${o.discountValue}%` : `${o.discountValue} ج`}</td><td className="p-5 text-center"><button onClick={() => { const u = offers.map(off => off.id === o.id ? {...off, active: !off.active} : off); setOffers(u); syncToCloud({ offers: u }); }} className={`px-4 py-1.5 rounded-xl font-bold text-xs ${o.active ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-500'}`}>{o.active ? 'فعال' : 'موقف'}</button></td><td className="p-5 text-center"><button onClick={() => { setDeleteConfig({ type: 'offer', id: o.id }); setActiveModal('delete'); }} className="text-rose-500 p-2 bg-rose-50 rounded-xl"><Trash2 size={15}/></button></td></tr>))}
                    </tbody></table></div>
                  </div>
                ) : currentRoute === 'hr' ? (
                  <div className="p-4 md:p-8 max-w-7xl mx-auto"><div className="flex justify-between items-center mb-8"><div className="flex items-center gap-3"><Users className="text-indigo-600 w-8 h-8"/><h2 className="text-3xl font-black dark:text-white">شؤون الموظفين</h2></div><div className="flex gap-2"><button onClick={() => { setFormData({ type: 'advance' }); setActiveModal('hrTransaction'); }} className="bg-amber-100 text-amber-700 px-4 py-2.5 rounded-xl font-bold flex gap-2"><Banknote size={17}/> سلفة/خصم</button><button onClick={() => openModal('employee')} className="bg-indigo-600 text-white px-4 py-2.5 rounded-xl font-bold flex gap-2"><Plus size={17}/> موظف</button></div></div><div className="bg-white dark:bg-slate-800 rounded-3xl border overflow-hidden"><table className="w-full text-right"><thead className="bg-slate-50 border-b font-bold text-sm"><tr><th className="p-5">الاسم</th><th className="p-5">الراتب</th><th className="p-5">الصافي</th><th className="p-5 text-center">كشف الحساب</th><th className="p-5 text-center">إجراء</th></tr></thead><tbody>{employees.map(emp => { const empAdv = hrTransactions.filter(t => t.empId === emp.id && t.type === 'advance').reduce((s, t) => s + t.amount, 0); const empDed = hrTransactions.filter(t => t.empId === emp.id && t.type === 'deduction').reduce((s, t) => s + t.amount, 0); return (<tr key={emp.id} className="border-b text-sm"><td className="p-5 font-black dark:text-white">{emp.name}</td><td className="p-5 font-bold">{emp.salary}</td><td className="p-5 font-black text-emerald-600">{emp.salary - empAdv - empDed}</td><td className="p-5 text-center"><button onClick={() => { setFormData({ employee: emp }); setActiveModal('empHistory'); }} className="bg-slate-100 text-slate-700 px-4 py-1.5 rounded-xl font-bold text-xs">عرض السجل</button></td><td className="p-5 text-center"><button onClick={() => { setDeleteConfig({ type: 'employee', id: emp.id }); setActiveModal('delete'); }} className="text-rose-500 p-2 bg-rose-50 rounded-xl"><Trash2 size={15}/></button></td></tr>);})}</tbody></table></div></div>
                ) : currentRoute === 'inventory' ? (
                  <div className="p-4 md:p-8 max-w-7xl mx-auto"><div className="flex justify-between items-center mb-8"><div className="flex items-center gap-3"><Package className="text-indigo-600 w-8 h-8"/><h2 className="text-3xl font-black dark:text-white">المواد الخام</h2></div><button onClick={() => openModal('material')} className="bg-indigo-600 text-white px-4 py-2.5 rounded-xl font-bold flex gap-2"><Plus size={17}/> مادة جديدة</button></div><div className="bg-white dark:bg-slate-800 rounded-3xl border overflow-hidden"><table className="w-full text-right"><thead className="bg-slate-50 border-b font-bold text-sm"><tr><th className="p-5">المادة</th><th className="p-5">الكمية</th><th className="p-5">التكلفة</th><th className="p-5">تاريخ الصلاحية</th><th className="p-5 text-center">إجراء</th></tr></thead><tbody>{rawMaterials.map(rm => { const isExp = rm.expiryDate && new Date(rm.expiryDate) < new Date(); return (<tr key={rm.id} className="border-b text-sm"><td className="p-5 font-black dark:text-white">{rm.name}</td><td className="p-5"><span className={`px-3 py-1 rounded-xl font-black text-xs ${rm.currentStock <= 5 ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700'}`}>{rm.currentStock} {rm.unit}</span></td><td className="p-5 font-bold">{rm.costPerUnit} ج</td><td className="p-5 font-bold"><span className={`px-3 py-1 rounded-xl text-xs ${isExp ? 'bg-rose-600 text-white font-black' : rm.expiryDate ? 'bg-slate-100 text-slate-700' : ''}`}>{rm.expiryDate || 'غير محدد'}</span></td><td className="p-5 text-center"><button onClick={() => { setDeleteConfig({ type: 'material', id: rm.id }); setActiveModal('delete'); }} className="text-rose-500 p-2 bg-rose-50 rounded-xl"><Trash2 size={15}/></button></td></tr>); })}</tbody></table></div></div>
                ) : 
                // باقي الشاشات العادية (داشبورد، فواتير، ورديات، منتجات، طاولات، مصروفات، جرد) ... 
                null}
              </div>
            </main>
            ...
            {activeModal === 'psDevice' && (
              <CustomModal title="جهاز بلايستيشن جديد" onClose={closeModal}>
                <form onSubmit={e => { e.preventDefault(); genericSave('psDevices', psDevices, setPsDevices, { name: e.target.name.value, hourlyRate: parseFloat(e.target.hourlyRate.value), status: 'available' }); }} className="space-y-4">
                  <input required name="name" placeholder="اسم الجهاز (مثال: جهاز 1, الغرفة VIP)" className="w-full p-4 border rounded-2xl font-bold dark:bg-slate-900 outline-none"/>
                  <input required type="number" step="any" name="hourlyRate" placeholder="سعر الساعة (ج)" className="w-full p-4 border rounded-2xl font-black dark:bg-slate-900 outline-none"/>
            ...
            {activeModal === 'psCheckout' && formData.device && (
              <CustomModal title={`دفع حساب - ${formData.device.name}`} onClose={closeModal}>
                <div className="text-center p-4">
                  <Gamepad className="w-16 h-16 text-indigo-600 mx-auto mb-4"/>
                  <h3 className="text-2xl font-black mb-4">إجمالي الحساب: {formData.cost} ج</h3>
            ...
            {activeModal === 'offer' && (
              <CustomModal title="إضافة عرض / خصم" onClose={closeModal}>
                <form onSubmit={e => { e.preventDefault(); genericSave('offers', offers, setOffers, { name: e.target.name.value, discountType: e.target.discountType.value, discountValue: parseFloat(e.target.discountValue.value), active: true }); }} className="space-y-4">
                  <input required name="name" placeholder="اسم العرض (مثال: خصم موظفين، افتتاح)" className="w-full p-4 border rounded-2xl font-bold dark:bg-slate-900 outline-none"/>
                  <div className="flex gap-4">
                    <select name="discountType" className="p-4 border rounded-2xl font-bold dark:bg-slate-900 outline-none"><option value="percent">نسبة مئوية (%)</option><option value="fixed">مبلغ ثابت (ج)</option></select>
                    <input required type="number" step="any" name="discountValue" placeholder="القيمة" className="w-full p-4 border rounded-2xl font-black dark:bg-slate-900 outline-none"/>
                  </div>
            ...
            {activeModal === 'material' && (
              <CustomModal title="مادة خام جديدة" onClose={closeModal}>
                <form onSubmit={e => { e.preventDefault(); genericSave('rawMaterials', rawMaterials, setRawMaterials, { name: e.target.name.value, unit: e.target.unit.value, currentStock: parseFloat(e.target.currentStock.value), costPerUnit: parseFloat(e.target.costPerUnit.value), expiryDate: e.target.expiryDate.value || null }); }} className="space-y-4">
                  <input required name="name" placeholder="الاسم" className="w-full p-4 border rounded-2xl font-bold dark:bg-slate-900 outline-none"/>
                  <select required name="unit" className="w-full p-4 border rounded-2xl font-bold dark:bg-slate-900 outline-none"><option value="جرام">جرام</option><option value="مللي">مللي</option><option value="قطعة">قطعة</option></select>
                  <div className="flex gap-4"><input required type="number" step="any" name="currentStock" placeholder="الرصيد الافتتاحي" className="flex-1 p-4 border rounded-2xl font-black dark:bg-slate-900 outline-none"/><input required type="number" step="any" name="costPerUnit" placeholder="التكلفة (ج)" className="flex-1 p-4 border rounded-2xl font-black dark:bg-slate-900 outline-none"/></div>
                  <div className="pt-2"><label className="block text-sm font-bold mb-2">تاريخ انتهاء الصلاحية (اختياري، للمنتجات سريعة التلف)</label><input type="date" name="expiryDate" className="w-full p-4 border rounded-2xl font-bold dark:bg-slate-900 outline-none"/></div>
            ...
            {activeModal === 'empHistory' && formData.employee && (
              <CustomModal title={`سجل الموظف: ${formData.employee.name}`} onClose={closeModal}>
                <div className="space-y-6">
                  <div><h4 className="font-black border-b pb-2 mb-3">السلف والخصومات</h4>{hrTransactions.filter(t => t.empId === formData.employee.id).length === 0 ? <p className="text-sm text-slate-500">لا يوجد سجل</p> : hrTransactions.filter(t => t.empId === formData.employee.id).map(t => <div key={t.id} className="flex justify-between text-sm p-2 border-b"><span>{t.date.split(',')[0]} - {t.reason}</span><span className={t.type==='advance' ? 'text-amber-600 font-bold' : 'text-rose-600 font-bold'}>{t.type==='advance'?'سلفة':'خصم'} ({t.amount} ج)</span></div>)}</div>
                  {formData.employee.hasAuth && <div><h4 className="font-black border-b pb-2 mb-3">الورديات اللي قفلها</h4>{shifts.filter(s => s.cashierName === formData.employee.name && s.status === 'closed').length === 0 ? <p className="text-sm text-slate-500">لم يقفل أي وردية</p> : shifts.filter(s => s.cashierName === formData.employee.name && s.status === 'closed').slice(-5).map(s => <div key={s.id} className="flex justify-between text-sm p-2 border-b"><span>{s.startTime.split(',')[0]}</span><span className="font-bold text-indigo-600">مبيعات: {s.totalSales} ج</span></div>)}</div>}
                </div>
              </CustomModal>
            )}
